...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	if (likely(pte_same(*vmf->pte, vmf->orig_pte))) {
		...
		__lru_cache_add_active_or_unevictable(new_page, get_fault_vma_flags(vmf));
		/*
	...
	if (old_page) {
		 ...
		 */
		if (page_copied && (get_fault_vma_flags(vmf) & VM_LOCKED)) {
...
{
	WARN_ON_ONCE(!(get_fault_vma_flags(vmf) & VM_SHARED));
	if (!pte_map_lock(vmf))
		return VM_FAULT_RETRY;
	/*
	...
	__releases(vmf->ptl)
{
	...
	if (!vmf->page) {
		 ...
		 */
		if ((get_fault_vma_flags(vmf) & (VM_WRITE|VM_SHARED)) ==
				     (VM_WRITE|VM_SHARED))
	...
	if (PageAnon(vmf->page)) {
		...
		unlock_page(vmf->page);
	} else if (unlikely((get_fault_vma_flags(vmf) & (VM_WRITE|VM_SHARED)) ==
					(VM_WRITE|VM_SHARED))) {
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	pte = mk_pte(page, get_fault_vma_prot(vmf));
	if ((vmf->flags & FAULT_FLAG_WRITE) && reuse_swap_page(page, NULL)) {
		pte = maybe_mkwrite(pte_mkdirty(pte), get_fault_vma_flags(vmf));
	...
	if (unlikely(page != swapcache && swapcache)) {
		...
		__lru_cache_add_active_or_unevictable(page, get_fault_vma_flags(vmf));
	} else {
	...
	if (mem_cgroup_swap_full(page) ||
	    (get_fault_vma_flags(vmf) & VM_LOCKED) || PageMlocked(page))
...
static vm_fault_t do_anonymous_page(struct vm_fault *vmf)
{
	...
	if (get_fault_vma_flags(vmf) & VM_SHARED)
		return VM_FAULT_SIGBUS;
...
static vm_fault_t do_set_pmd(struct vm_fault *vmf, struct page *page)
{
	...
	entry = mk_huge_pmd(page, get_fault_vma_prot(vmf));
	if (write)
		...
		struct page *page)
{
	...
	if (write)
		entry = maybe_mkwrite(pte_mkdirty(entry), get_fault_vma_flags(vmf));
	/* copy-on-write page */
	if (write && !(get_fault_vma_flags(vmf) & VM_SHARED)) {
		inc_mm_counter_fast(vma->vm_mm, MM_ANONPAGES);
		__page_add_new_anon_rmap(page, vma, vmf->address, false);
		mem_cgroup_commit_charge(page, memcg, false, false);
		__lru_cache_add_active_or_unevictable(page, get_fault_vma_flags(vmf));
...
vm_fault_t finish_fault(struct vm_fault *vmf)
{
	...
	/* Did we COW the page? */
	if ((vmf->flags & FAULT_FLAG_WRITE) &&
	    !(get_fault_vma_flags(vmf) & VM_SHARED))
		page = vmf->cow_page;
	else
		page = vmf->page;
...
static vm_fault_t do_fault(struct vm_fault *vmf)
{
	...
	else if (!(get_fault_vma_flags(vmf) & VM_SHARED))
...
static vm_fault_t do_numa_page(struct vm_fault *vmf)
{
	...
	pte = pte_modify(old_pte, get_fault_vma_prot(vmf));
	pte = pte_mkyoung(pte);
	if (was_writable)
		pte = pte_mkwrite(pte);
	...
	page = _vm_normal_page(vma, vmf->address, pte, get_fault_vma_flags(vmf));
	if (!page) {
		pte_unmap_unlock(vmf->pte, vmf->ptl);
		return 0;
	}
	 ...
	 */
	if (page_mapcount(page) > 1 && (get_fault_vma_flags(vmf) & VM_SHARED))
		flags |= TNF_SHARED;
...
static inline vm_fault_t wp_huge_pmd(struct vm_fault *vmf, pmd_t orig_pmd)
{
	...
	VM_BUG_ON_VMA(get_fault_vma_flags(vmf) & VM_SHARED, vmf->vma);
