...
/*
 *  Sound Machine Driver for Prince AMPs on Unbound
 *
...
static struct _drvdata unbound_drvdata;
...
static int unbound_uaif2_init(struct snd_soc_pcm_runtime *rtd)
{
...
static int unbound_dai_ops_hw_params(struct snd_pcm_substream *substream,
...
static const struct snd_soc_ops uaif_ops = {
	.hw_params = unbound_dai_ops_hw_params,
};
...
static int unbound_late_probe(struct snd_soc_card *card)
{
...
static const struct snd_soc_dapm_widget unbound_supply_widgets[] = {
...
static int unbound_probe(struct snd_soc_card *card)
{
	struct _drvdata *drvdata = card->drvdata;
	int i;

	for (i = 0; i < ARRAY_SIZE(unbound_supply_widgets); i++) {
		const struct snd_soc_dapm_widget *w;
		struct regulator *r;

		w = &unbound_supply_widgets[i];
		switch (w->id) {
	...
	drvdata->ws = wakeup_source_register(NULL, "unbound-sound");

	return 0;
}

static int unbound_remove(struct snd_soc_card *card)
{
	struct _drvdata *drvdata = card->drvdata;
...
static struct snd_soc_dai_link unbound_dai[100] = {
	{
		.name = "RDMA0",
		.stream_name = "RDMA0",
		.dynamic = 1,
...
static int unbound_dmic1(struct snd_soc_dapm_widget *w,
...
static int unbound_dmic2(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_dmic3(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_l_speaker(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_r_speaker(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_bt_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_bt_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_usb_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_usb_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_fwd_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int unbound_fwd_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
{
	struct _drvdata *drvdata = &unbound_drvdata;
...
static const struct snd_kcontrol_new unbound_controls[] = {
...
static const struct snd_soc_dapm_widget unbound_widgets[] = {
	SND_SOC_DAPM_MIC("DMIC1", unbound_dmic1),
	SND_SOC_DAPM_MIC("DMIC2", unbound_dmic2),
	SND_SOC_DAPM_MIC("DMIC3", unbound_dmic3),
	SND_SOC_DAPM_SPK("RECEIVER", unbound_l_speaker),
	SND_SOC_DAPM_SPK("SPEAKER", unbound_r_speaker),
	SND_SOC_DAPM_MIC("BLUETOOTH MIC", unbound_bt_mic),
	SND_SOC_DAPM_SPK("BLUETOOTH SPK", unbound_bt_out),
	SND_SOC_DAPM_MIC("USB MIC", unbound_usb_mic),
	SND_SOC_DAPM_SPK("USB SPK", unbound_usb_out),
	SND_SOC_DAPM_MIC("FWD MIC", unbound_fwd_mic),
	SND_SOC_DAPM_SPK("FWD SPK", unbound_fwd_out),
	SND_SOC_DAPM_OUTPUT("VTS Virtual Output"),
	SND_SOC_DAPM_MUX("VTS Virtual Output Mux", SND_SOC_NOPM, 0, 0,
			&vts_output_mux[0]),
};

static const struct snd_soc_dapm_route unbound_routes[] = {
...
static struct snd_soc_card unbound_prince = {
	.name = "Unbound-Prince",
	.owner = THIS_MODULE,
	.dai_link = unbound_dai,
	.num_links = ARRAY_SIZE(unbound_dai),

	.probe = unbound_probe,
	.late_probe = unbound_late_probe,
	.remove = unbound_remove,

	.controls = unbound_controls,
	.num_controls = ARRAY_SIZE(unbound_controls),
	.dapm_widgets = unbound_widgets,
	.num_dapm_widgets = ARRAY_SIZE(unbound_widgets),
	.dapm_routes = unbound_routes,
	.num_dapm_routes = ARRAY_SIZE(unbound_routes),

	.drvdata = (void *)&unbound_drvdata,
...
static void unbound_register_card_work_func(struct work_struct *work)
{
	struct snd_soc_card *card = &unbound_prince;
	int ret;
...
DECLARE_WORK(unbound_register_card_work, unbound_register_card_work_func);

static int unbound_audio_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &unbound_prince;
	struct _drvdata *drvdata = card->drvdata;
	...
	schedule_work(&unbound_register_card_work);

	return ret;
}

static int unbound_audio_remove(struct platform_device *pdev)
{
	...
	for (dai_link = unbound_dai; dai_link - unbound_dai <
			ARRAY_SIZE(unbound_dai); dai_link++) {
...
static const struct of_device_id unbound_audio_of_match[] = {
	{ .compatible = "samsung,unbound-prince", },
	{},
};
MODULE_DEVICE_TABLE(of, unbound_audio_of_match);
#endif /* CONFIG_OF */

static struct platform_driver unbound_audio_driver = {
	.driver		= {
		.name	= "unbound-audio",
		.owner	= THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = of_match_ptr(unbound_audio_of_match),
	},

	.probe		= unbound_audio_probe,
	.remove		= unbound_audio_remove,
};

module_platform_driver(unbound_audio_driver);

MODULE_DESCRIPTION("ALSA SoC Unbound Audio Driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:unbound-prince");
