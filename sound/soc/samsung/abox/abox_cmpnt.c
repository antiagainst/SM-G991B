...
static bool sifs_channel_selected(struct abox_data *data, int idx)
{
	...
	snd_soc_component_read(data->cmpnt, reg, &val);
...
static void update_ch_num(struct device *adev, enum ABOX_CONFIGMSG configmsg)
{
	...
	case SET_SIFS0_FORMAT ... SET_SIFS6_FORMAT:
		...
		break;
	case SET_SIFM0_FORMAT ... SET_SIFM7_FORMAT:
		/* update output channels */
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = data->debug_mode;

	return 0;
}

static int debug_put_ipc(struct device *dev, bool debug)
{
	struct abox_data *data = dev_get_drvdata(dev);
	ABOX_IPC_MSG msg;
	struct IPC_SYSTEM_MSG *system_msg = &msg.msg.system;
	...
	system_msg->param1 = !data->debug_mode;
...
static const struct snd_kcontrol_new cmpnt_controls[] = {
	...
	SOC_SINGLE_EXT("Debug", 0, 0, 1, 0, debug_get, debug_put),
	SOC_SINGLE_EXT("Wakelock", 0, 0, 1, 0, wake_lock_get, wake_lock_put),
	SOC_SINGLE_EXT("Reset Log", 0, 0, 1, 0, reset_log_get, reset_log_put),
	SOC_SINGLE("NSRC0 Bridge", ABOX_ROUTE_CTRL_CONNECT,
			ABOX_NSRC_CONNECTION_TYPE_L(0), 1, 0),
	SOC_SINGLE("NSRC1 Bridge", ABOX_ROUTE_CTRL_CONNECT,
			ABOX_NSRC_CONNECTION_TYPE_L(1), 1, 0),
	SOC_SINGLE("NSRC2 Bridge", ABOX_ROUTE_CTRL_CONNECT,
			ABOX_NSRC_CONNECTION_TYPE_L(2), 1, 0),
	SOC_SINGLE("NSRC3 Bridge", ABOX_ROUTE_CTRL_CONNECT,
			ABOX_NSRC_CONNECTION_TYPE_L(3), 1, 0),
	SOC_SINGLE("NSRC4 Bridge", ABOX_ROUTE_CTRL_CONNECT,
			ABOX_NSRC_CONNECTION_TYPE_L(4), 1, 0),
	SOC_SINGLE("NSRC5 Bridge", ABOX_ROUTE_CTRL_CONNECT,
			ABOX_NSRC_CONNECTION_TYPE_L(5), 1, 0),
	SOC_SINGLE("NSRC6 Bridge", ABOX_ROUTE_CTRL_CONNECT,
			ABOX_NSRC_CONNECTION_TYPE_L(6), 1, 0),
	SOC_SINGLE("NSRC7 Bridge", ABOX_ROUTE_CTRL_CONNECT,
...
static enum abox_dai spus_sink[ABOX_RDMA11 - ABOX_RDMA0 + 1];
		...
		enum abox_dai dai)
{
	...
	case ABOX_RDMA10_BE:
	case ABOX_RDMA11_BE:
		spus_sink[spus - ABOX_RDMA0_BE] = dai;
		break;
	default:
		abox_err(dev, "%s: invalid dai %d\n", __func__, spus);
		ret = -EINVAL;
		break;
	}
		...
		enum abox_dai spus)
{
	...
	case ABOX_RDMA10_BE:
	case ABOX_RDMA11_BE:
		dai = spus_sink[spus - ABOX_RDMA0_BE];
		break;
	default:
		abox_err(dev, "%s: invalid dai %d\n", __func__, spus);
		break;
...
static unsigned int nsrc_source[8];
		...
		enum abox_dai dai)
{
	...
	case ABOX_NSRC6:
	case ABOX_NSRC7:
		nsrc_source[nsrc - ABOX_NSRC0] = dai;
		break;
	default:
		abox_err(dev, "%s: invalid dai %d\n", __func__, nsrc);
		ret = -EINVAL;
		...
		enum abox_dai nsrc)
{
	...
	case ABOX_NSRC6:
	case ABOX_NSRC7:
		dai = nsrc_source[nsrc - ABOX_NSRC0];
		break;
	default:
		abox_err(dev, "%s: invalid dai %d\n", __func__, nsrc);
		break;
		...
		enum abox_dai dai)
{
	...
	case ABOX_NSRC7:
		idx = dai - ABOX_NSRC0;
		reg = ABOX_ROUTE_CTRL_CONNECT;
		snd_soc_component_read(cmpnt, reg, &val);
		val &= ABOX_NSRC_CONNECTION_TYPE_MASK(idx);
		val >>= ABOX_NSRC_CONNECTION_TYPE_L(idx);
		ret = !val;
		break;
	default:
		ret = false;
...
static enum abox_dai get_source_dai_id(struct abox_data *data, enum abox_dai id)
{
	...
	case ABOX_UAIF6:
		...
		break;
	case ABOX_DSIF:
		snd_soc_component_read(cmpnt, ABOX_ROUTE_CTRL_DSIF, &val);
	...
	case ABOX_RDMA11_BE:
		...
		break;
	case ABOX_SIFSM:
		snd_soc_component_read(cmpnt, ABOX_ROUTE_CTRL_SIFM, &val);
		...
		break;
	case ABOX_SIFST:
		snd_soc_component_read(cmpnt, ABOX_ROUTE_CTRL_SIFT, &val);
...
static enum abox_dai get_sink_dai_id(struct abox_data *data, enum abox_dai id)
{
	...
	case ABOX_UAIF6:
	case ABOX_SPDY:
		for (_id = ABOX_NSRC0; _id <= ABOX_NSRC7; _id++) {
		...
		struct snd_kcontrol *k, int e)
{
	...
	return notify_event(data, ABOX_WIDGET_NSRC5, e);
}
		...
		struct snd_kcontrol *k, int e)
{
	...
	return notify_event(data, ABOX_WIDGET_NSRC6, e);
}
		...
		struct snd_kcontrol *k, int e)
{
	...
	return notify_event(data, ABOX_WIDGET_NSRC7, e);
}
...
static SOC_ENUM_SINGLE_DECL(spusm_enum, ABOX_ROUTE_CTRL_SIFM,
		ABOX_ROUTE_SPUSM_L, spusm_texts);
static const struct snd_kcontrol_new spusm_controls[] = {
	SOC_DAPM_ENUM("MUX", spusm_enum),
};

static SOC_ENUM_SINGLE_DECL(spust_enum, ABOX_ROUTE_CTRL_SIFT,
		ABOX_ROUTE_SPUST_L, spusm_texts);
		...
		enum abox_dai dai)
{
	...
	case ABOX_SIFST:
		...
		case ABOX_UAIF0 ... ABOX_UAIF6:
			...
			ret = snd_soc_component_read(cmpnt, reg, &reg_val);
			if (ret < 0)
				break;
			...
			break;
		case ABOX_BI_PDI0 ... ABOX_BI_PDI7:
			idx = src - ABOX_BI_PDI0;
			reg = ABOX_SW_PDI_CTRL_BI_PDI(idx);
			ret = snd_soc_component_read(cmpnt, reg, &reg_val);
			if (ret < 0)
				break;
...
static const char * const nsrcx_texts[] = {
	...
	"BI_PDI0", "BI_PDI1", "BI_PDI2", "BI_PDI3",
	"BI_PDI4", "BI_PDI5", "BI_PDI6", "BI_PDI7",
	"RX_PDI0", "RX_PDI1", "UDMA_SIFS0", "UDMA_SIFS1",
	"SPDY",
};
...
static const enum abox_dai nsrcx_dai[] = {
	...
	ABOX_BI_PDI0, ABOX_BI_PDI1, ABOX_BI_PDI2, ABOX_BI_PDI3,
	ABOX_BI_PDI4, ABOX_BI_PDI5, ABOX_BI_PDI6, ABOX_BI_PDI7,
	ABOX_RX_PDI0, ABOX_RX_PDI1, ABOX_UDMA_RD0, ABOX_UDMA_RD1,
	ABOX_SPDY,
};
		...
		struct snd_kcontrol *k, int e, int id)
{
	...
	return notify_event(data, ABOX_WIDGET_UDMA_RD0, e);
}
...
static const char * const sifms_texts[] = {
	"SIFM0", "SIFM1", "SIFM2", "SIFM3", "SIFM4", "SIFM5", "SIFM6", "SIFM7",
};
...
static const struct snd_soc_dapm_widget cmpnt_widgets[] = {
	...
	SND_SOC_DAPM_MIXER_E("UDMA SIFS1", SND_SOC_NOPM, 0, 0, NULL, 0,
			udma_sifs1_event,
			SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX("UDMA SIFM0", SND_SOC_NOPM, 0, 0, udma_sifm0_controls),
	SND_SOC_DAPM_MUX("UDMA SIFM1", SND_SOC_NOPM, 0, 0, udma_sifm1_controls),
};
...
static const struct snd_soc_dapm_route cmpnt_routes[] = {
	...
	{"SIFM0", NULL, "SPUM PGA0"},
	{"SIFM1", NULL, "SPUM PGA1"},
	{"SIFM2", NULL, "SPUM PGA2"},
	{"SIFM3", NULL, "SPUM PGA3"},
	{"SIFM4", NULL, "SPUM PGA4"},
	{"SIFM5", NULL, "SPUM PGA5"},
	{"SIFM6", NULL, "SPUM PGA6"},
	{"SIFM7", NULL, "SPUM PGA7"},
...
{
	struct device *dev = dai->dev;
	enum abox_dai id = dai->id, sink_id;
	struct abox_data *data = dev_get_drvdata(dev);
	int ret = 0;
	...
	sink_id = get_sink_dai_id(data, id);
	if (sink_id >= ABOX_UAIF0 && sink_id <= ABOX_DSIF)
		goto out;
