...
/**
 * vts_irq_gc_noop - NOOP function
 * @d: irq_data
 */
void vts_irq_gc_noop(struct irq_data *d)
{
}
 ...
 * register.
 */
void vts_irq_gc_mask_disable_reg(struct irq_data *d)
{
...
/**
 * vts_irq_gc_unmask_enable_reg - Unmask chip via enable register
 * @d: irq_data
 *
 * Chip has separate enable/disable registers instead of a single mask
 * register.
 */
void vts_irq_gc_unmask_enable_reg(struct irq_data *d)
{
	struct irq_chip_generic *gc = irq_data_get_irq_chip_data(d);
	struct irq_chip_type *ct = irq_data_get_chip_type(d);
	u32 mask = d->mask;
...
/**
 * vts_irq_gc_ack_clr_bit - Ack pending interrupt via clearing bit
 * @d: irq_data
 */
void vts_irq_gc_ack_clr_bit(struct irq_data *d)
{
	struct irq_chip_generic *gc = irq_data_get_irq_chip_data(d);
	struct irq_chip_type *ct = irq_data_get_chip_type(d);
	u32 mask = ~d->mask;
...
/**
 * vts_irq_gc_mask_disable_and_ack_set - Mask and ack pending interrupt
 * @d: irq_data
 *
 ...
 */
void vts_irq_gc_mask_disable_and_ack_set(struct irq_data *d)
{
	struct irq_chip_generic *gc = irq_data_get_irq_chip_data(d);
	struct irq_chip_type *ct = irq_data_get_chip_type(d);
	u32 mask = d->mask;
...
/**
 * vts_irq_gc_eoi - EOI interrupt
 * @d: irq_data
 */
void vts_irq_gc_eoi(struct irq_data *d)
{
	struct irq_chip_generic *gc = irq_data_get_irq_chip_data(d);
	struct irq_chip_type *ct = irq_data_get_chip_type(d);
	u32 mask = d->mask;
...
/**
 * vts_irq_gc_set_wake - Set/clr wake bit for an interrupt
 * @d:  irq_data
 ...
 */
int vts_irq_gc_set_wake(struct irq_data *d, unsigned int on)
{
	struct irq_chip_generic *gc = irq_data_get_irq_chip_data(d);
	u32 mask = d->mask;
...
vts_irq_alloc_generic_chip(const char *name, int num_ct, unsigned int irq_base,
		       void __iomem *reg_base, irq_flow_handler_t handler)
{
	...
	if (gc) {
		vts_irq_init_generic_chip(gc, name, num_ct, irq_base, reg_base,
				      handler);
	}
	return gc;
}
EXPORT_SYMBOL_GPL(vts_irq_alloc_generic_chip);
 ...
 * associated handler.
 */
void vts_irq_setup_generic_chip(struct irq_chip_generic *gc, u32 msk,
			    ...
			    unsigned int set)
{
	...
	vts_irq_gc_init_mask_cache(gc, flags);
...
EXPORT_SYMBOL_GPL(vts_irq_setup_generic_chip);
 ...
 */
int vts_irq_setup_alt_chip(struct irq_data *d, unsigned int type)
{
	struct irq_chip_generic *gc = irq_data_get_irq_chip_data(d);
	struct irq_chip_type *ct = gc->chip_types;
	unsigned int i;
...
EXPORT_SYMBOL_GPL(vts_irq_setup_alt_chip);
