...
void sc_hwregs_dump(struct sc_dev *sc)
{
	...
	if (sc->version >= SCALER_VERSION(6, 0, 1))
		sc_print_hex_dump(sc, sc->regs + 0x310, 0x314 - 0x310 + 4);
...
void sc_hwset_init_votf(struct sc_dev *sc)
{
	...
	for (tws_idx = 0; tws_idx < SC_NUM_TWS_IDX; tws_idx++) {
		unsigned int tws_offset;

		tws_offset = tws_idx * MSCL_VOTF_TWS_OFFSET;

		writel_relaxed(MSCL_VOTF_EN_VAL, sc->votf_regs + tws_offset +
						 MSCL_VOTF_TWS_ENABLE);
		writel_relaxed(0xff, sc->votf_regs + tws_offset +
				     MSCL_VOTF_TWS_LIMIT);
...
void sc_hwset_votf(struct sc_dev *sc, struct sc_votf *votf)
{
	void __iomem *dpu_dma_base = sc->votf_table[votf->dpu_dma_idx].regs;
	u32 dpu_votf_base_pa = sc->votf_table[votf->dpu_dma_idx].votf_base_pa;
	unsigned int trs_idx = votf->trs_idx % SC_VOTF_NUM_TRS_PER_DPU;
	unsigned int buf_idx = votf->buf_idx;
	unsigned int tws_idx = (sc->prev_tws_idx == 1) ? 0 : 1;
	unsigned int tws_offset = tws_idx * MSCL_VOTF_TWS_OFFSET;
	u32 cfg;

	cfg = readl_relaxed(sc->regs + SCALER_CFG);
	writel_relaxed(cfg | SCALER_CFG_VOTF_EN, sc->regs + SCALER_CFG);

	writel_relaxed(tws_idx, sc->regs + SCALER_VOTF_MASTER_ID);
	writel_relaxed(MSCL_VOTF_HALF_CONNECTION,
		       sc->votf_regs + MSCL_VOTF_CONNECTION_MODE);
	writel_relaxed((((dpu_votf_base_pa & 0xffff0000) >> 16) << 4) | trs_idx,
		       sc->votf_regs + MSCL_VOTF_TWS_DEST_ID + tws_offset);

	writel_relaxed(buf_idx, dpu_dma_base + SC_DPU_BUF_WRITE_COUNTER +
		       trs_idx * SC_DPU_LAYER_OFFSET);

	sc->prev_tws_idx = tws_idx;
}
...
void sc_hwset_clear_votf_clock_en(struct sc_dev *sc)
{
	unsigned int tws_offset = sc->prev_tws_idx * MSCL_VOTF_TWS_OFFSET;
	int count = 100;

	while (count-- > 0)
		if (!readl_relaxed(sc->votf_regs + tws_offset +
				   MSCL_VOTF_TWS_BUSY))
			break;
	...
	if (count <= 0) {
		struct sc_votf *votf = sc->current_ctx->votf;
		unsigned int tws_idx = sc->prev_tws_idx;
		unsigned int trs_idx = votf->trs_idx;
		unsigned int dpu_dma_idx = votf->dpu_dma_idx;
		unsigned int buf_idx = votf->buf_idx;
		unsigned int tws_offset = tws_idx * MSCL_VOTF_TWS_OFFSET;

		dev_err(sc->dev, "TWS hasn't completed for 1 ms\n");
		sc_hwregs_dump(sc);
		sc_ctx_dump(sc->current_ctx);
		dev_err(sc->dev,
	...
	writel_relaxed(0x0, sc->votf_regs + MSCL_VOTF_RING_CLOCK_EN);
}
