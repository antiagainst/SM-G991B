...
static int jsqz_clock_gating(struct jsqz_dev *jsqz, bool on)
{
	...
	if (!IS_ERR(jsqz->clk_producer)) {
		if (on) {
			...
			if (ret) {
				dev_err(jsqz->dev, "%s failed to enable clock : %d\n"
					, __func__, ret);
				return ret;
			}
...
#else
static int jsqz_sysmmu_fault_handler(struct iommu_fault *fault, void *data)
{
	struct jsqz_dev *jsqz = data;
...
#if 0 //defined(CONFIG_PM_DEVFREQ)
...
#else
	iommu_register_device_fault_handler(&pdev->dev, jsqz_sysmmu_fault_handler, jsqz);
#endif
