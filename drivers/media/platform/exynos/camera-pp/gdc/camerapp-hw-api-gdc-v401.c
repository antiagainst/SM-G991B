...
#include "camerapp-hw-reg-gdc-v401.h"
#include <linux/smc.h>
...
{
	camerapp_sfr_set_reg(base_addr, &gdc_regs[GDC_R_GDC_INT_STATUS], 0x003F);
}
						...
						struct gdc_frame *d_frame, struct gdc_crop_param *crop_param)
{
	...
	if (gdc_crop_width * MAX_OUTPUT_SCALER_RATIO < gdc_output_width) {
		...
		out_scaled_width = 8192 * gdc_input_width / gdc_output_width;
	}
	...
	if (gdc_crop_height * MAX_OUTPUT_SCALER_RATIO < gdc_output_height) {
		...
		out_scaled_height = 8192 * gdc_input_height / gdc_output_height;
	}
...
void camerapp_hw_gdc_update_param(void __iomem *base_addr, struct gdc_dev *gdc)
{
	...
	camerapp_sfr_set_field(base_addr, &gdc_regs[GDC_R_GDC_BOUNDARY_OPTION],
...
void camerapp_hw_gdc_votf_enable(void __iomem *base_addr, u8 rw)
{
	if (rw) {
		/* read */
		camerapp_sfr_set_reg(base_addr, &gdc_regs[GDC_R_GDC_VOTF], 1);
	} else {
		/* write */
		camerapp_sfr_set_reg(base_addr, &gdc_regs[GDC_R_GDC_VOTF], 0x18);
		camerapp_sfr_set_reg(base_addr, &gdc_regs[GDC_R_GDC_BUS_USER_INFO_RDMA], 0);
		camerapp_sfr_set_reg(base_addr, &gdc_regs[GDC_R_GDC_BUS_USER_INFO_WDMA], 0x40);
	}
}
