...
#include "pd_mgr/camerapp-pd-mgr.h"
	...
	for (i = 0; i < num_planes; ++i) {
		...
		if (gdc_fmt_is_ayv12(gdc_fmt->pixelformat)) {
			...
			if ((gdc_fmt->pixelformat == V4L2_PIX_FMT_NV12M_S10B)
				...
				if ((i % 2) == 0) {
					pixm->plane_fmt[i].sizeimage =
						SBWCL_8B_Y_SIZE(pixm->width, pixm->height, GDC_LOSSY_COMP_RATE_50);
				} else if ((i % 2) == 1) {
					pixm->plane_fmt[i].sizeimage =
						SBWCL_8B_CBCR_SIZE(pixm->width, pixm->height, GDC_LOSSY_COMP_RATE_50);
				}
			} else if (gdc_fmt->pixelformat == V4L2_PIX_FMT_NV12M_SBWCL_10B) {
				if ((i % 2) == 0) {
					pixm->plane_fmt[i].sizeimage =
						SBWCL_10B_Y_SIZE(pixm->width, pixm->height, GDC_LOSSY_COMP_RATE_60);
				} else if ((i % 2) == 1) {
					pixm->plane_fmt[i].sizeimage =
						SBWCL_10B_CBCR_SIZE(pixm->width, pixm->height, GDC_LOSSY_COMP_RATE_60);
				}
			} else {
				pixm->plane_fmt[i].sizeimage =
					pixm->plane_fmt[i].bytesperline * pixm->height;
			     ...
			     enum v4l2_buf_type type)
{
	...
	if (!V4L2_TYPE_IS_OUTPUT(type)) {
		...
		if (gdc->scenario == GDC_SCEN_VOTF && gdc->has_votf_mfc == 1) {
			ret = camerapp_pd_mgr_on();
			if (ret < 0) {
				dev_err(gdc->dev, "gdc camerapp pd mgr on fail (ret:%d)\n", ret);
	...
	if (!V4L2_TYPE_IS_OUTPUT(type)) {
		if (gdc->scenario == GDC_SCEN_VOTF && gdc->has_votf_mfc == 1) {
			...
			if (camerapp_pd_mgr_off() < 0)
				dev_err(gdc->dev, "gdc camerapp_pd_mgr_off fail\n");
...
static int gdc_v4l2_s_input(struct file *file, void *fh,
	unsigned int input)
{
	int ret = 0;
	struct gdc_ctx *ctx = fh_to_gdc_ctx(fh);
	...
	gdc->scenario = (input & GDC_SCENARIO_MASK) >> GDC_SCENARIO_SHIFT;
...
static const struct v4l2_ioctl_ops gdc_v4l2_ioctl_ops = {
	...
	.vidioc_s_input 		= gdc_v4l2_s_input,
...
static void gdc_watchdog(struct timer_list *t)
{
	...
	if (gdc->scenario == GDC_SCEN_VOTF && gdc->has_votf_mfc == 1) {
		if (atomic_read(&gdc->f_frame)) {
			gdc_dbg("Waiting for MFC to be ready\n");
...
int gdc_votf_create_link(void __iomem *base_addr, struct gdc_dev *gdc)
{
	...
	case GDC_SCEN_VOTF:
		...
		if (ret < 0) {
			gdc_dbg("%s create link error (ret:%d)\n"
				,__func__, ret);
			return ret;
		}
		camerapp_hw_gdc_votf_enable(base_addr, TWS);
		break;
	default: /* GDC_SCEN_NORMAL */
		break;
	}
	return ret;
}
...
static int gdc_run_next_job(struct gdc_dev *gdc)
{
	...
	if (gdc->scenario == GDC_SCEN_VOTF && gdc->has_votf_mfc == 1) {
		...
		if (ret < 0) {
			dev_err(gdc->dev, "gdc votf create link fail (ret:%d)\n", ret);
			return ret;
		}
	}
...
static irqreturn_t gdc_irq_handler(int irq, void *priv)
{
	...
	if (irq_status & GDC_INT_FRAME_START) {
	...
	if (irq_status & GDC_INT_FRAME_END) {
		clear_bit(DEV_RUN, &gdc->state);
		del_timer(&gdc->wdt.timer);
		atomic_set(&gdc->f_frame, 0);
		atomic_set(&gdc->wdt.cnt, 0);
	...
	if (irq_status & GDC_INT_VOTF_SLOW_RING_M)
		dev_err(gdc->dev, "GDC_INT_VOTF_SLOW_RING_M\n");

	if (irq_status & GDC_INT_VOTF_LOST_FLUSH_M)
		dev_err(gdc->dev, "GDC_INT_VOTF_LOST_FLUSH_M\n");
...
static ssize_t gdc_s_scenario_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct gdc_dev *gdc =
		(struct gdc_dev *)dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "scenario:%d\n", gdc->scenario);
}

static ssize_t gdc_s_scenario_store(struct device *dev,
				 ...
				 const char *buf, size_t count)
{
	...
	if (buf[0] == '1')
		gdc->scenario = GDC_SCEN_VOTF;
	else
		gdc->scenario = GDC_SCEN_NORMAL;

	gdc_dbg("%s : scenario: %d\n", __func__, gdc->scenario);

	return count;
}

static DEVICE_ATTR(s_scenario, 0644, gdc_s_scenario_show, gdc_s_scenario_store);

static struct attribute *gdc_entries[] = {
	&dev_attr_s_scenario.attr,
	NULL,
};

static struct attribute_group gdc_attr_group = {
	.name	= "gdc",
	.attrs	= gdc_entries,
};
...
*/
	...
	ret = sysfs_create_group(&pdev->dev.kobj, &gdc_attr_group);
	if (ret < 0) {
