...
void tsmux_clear_partial_done(struct tsmux_device *tsmux_dev, u32 job_id)
{
	...
	print_tsmux(TSMUX_SFR, "write int_stat_reg 0x%x\n", int_stat_reg);
	TSMUX_WRITEL(int_stat_reg, TSMUX_INT_STAT_ADDR);
	int_stat_reg = TSMUX_READL(TSMUX_INT_STAT_ADDR);
	print_tsmux(TSMUX_SFR, "read int_stat_reg 0x%x\n", int_stat_reg);
		...
		struct tsmux_device *tsmux_dev, struct tsmux_reg *cmd, int count)
{
	...
	for (i = 0; i < count; i++) {
		print_tsmux(TSMUX_SFR, "addr 0x%.8x, value 0x%.8x\n", cmd[i].addr, cmd[i].value);
		...
		struct tsmux_device *tsmux_dev, struct tsmux_reg *cmd, int count)
{
	...
	print_tsmux(TSMUX_SFR, "%s++: count: %d\n", __func__, count);

	if (tsmux_dev == NULL)
		return;

	for (i = 0; i < count; i++) {
		print_tsmux(TSMUX_SFR, "addr 0x%.8x, value 0x%.8x, i: %d\n", cmd[i].addr, cmd[i].value, i);
		ret = exynos_smc(0x82004007, TSMUX_HEX_BASE_ADDR + cmd[i].addr, cmd[i].value, 0);
		print_tsmux(TSMUX_SFR, "smc call ret: %x\n", ret);
	}
