...
static int is_lme_video_open(struct file *file)
{
	int ret = 0;
	int ret_err = 0;
	struct is_video *video;
	...
	snprintf(name, sizeof(name), "LME");
	ret = open_vctx(file, video, &vctx, device->instance, ENTRY_LME, name);
	if (ret) {
	...
	ret = is_ischain_lme_open(device, vctx);
	if (ret) {
		merr("is_ischain_lme_open is fail(%d)", device, ret);
