...
int is_sec_parse_rom_info(struct is_rom_info *finfo, char *buf, int rom_id)
{
	...
	if ( finfo->rom_ois_list_len == IS_ROM_OIS_MAX_LIST) {
		...
		if (rom_id == TELE_OIS_ROM_ID) {
			...
			memcpy(ois_pinfo->tele_romdata.ycoef, &buf[finfo->rom_ois_list[10]], IS_OIS_COEF_DATA_SIZE);
			memcpy(ois_pinfo->tele_romdata.supperssion_xratio, &buf[finfo->rom_ois_list[11]], IS_OIS_SUPPERSSION_RATIO_DATA_SIZE);
			memcpy(ois_pinfo->tele_romdata.supperssion_yratio, &buf[finfo->rom_ois_list[12]], IS_OIS_SUPPERSSION_RATIO_DATA_SIZE);
			memcpy(ois_pinfo->tele_romdata.cal_mark, &buf[finfo->rom_ois_list[13]], IS_OIS_CAL_MARK_DATA_SIZE);
		}
		...
		if (rom_id == TELE_OIS_TILT_ROM_ID) {
			...
			memcpy(ois_pinfo->tele_tilt_romdata.ycoef, &buf[finfo->rom_ois_list[10]], IS_OIS_COEF_DATA_SIZE);
			memcpy(ois_pinfo->tele_tilt_romdata.supperssion_xratio, &buf[finfo->rom_ois_list[11]], IS_OIS_SUPPERSSION_RATIO_DATA_SIZE);
			memcpy(ois_pinfo->tele_tilt_romdata.supperssion_yratio, &buf[finfo->rom_ois_list[12]], IS_OIS_SUPPERSSION_RATIO_DATA_SIZE);
			memcpy(ois_pinfo->tele_tilt_romdata.cal_mark, &buf[finfo->rom_ois_list[13]], IS_OIS_CAL_MARK_DATA_SIZE);
		}
		...
		if (rom_id == TELE_OIS_ROM_ID) {
			...
			memcpy(ois_pinfo->tele_romdata.ycoef, &buf[finfo->rom_ois_list[3]], IS_OIS_COEF_DATA_SIZE);
			memcpy(ois_pinfo->tele_romdata.supperssion_xratio, &buf[finfo->rom_ois_list[4]], IS_OIS_SUPPERSSION_RATIO_DATA_SIZE);
			memcpy(ois_pinfo->tele_romdata.supperssion_yratio, &buf[finfo->rom_ois_list[5]], IS_OIS_SUPPERSSION_RATIO_DATA_SIZE);
			memcpy(ois_pinfo->tele_romdata.cal_mark, &buf[finfo->rom_ois_list[6]], IS_OIS_CAL_MARK_DATA_SIZE);
		}
		...
		if (rom_id == TELE_OIS_TILT_ROM_ID) {
			...
			memcpy(ois_pinfo->tele_tilt_romdata.ycoef, &buf[finfo->rom_ois_list[3]], IS_OIS_COEF_DATA_SIZE);
			memcpy(ois_pinfo->tele_tilt_romdata.supperssion_xratio, &buf[finfo->rom_ois_list[4]], IS_OIS_SUPPERSSION_RATIO_DATA_SIZE);
			memcpy(ois_pinfo->tele_tilt_romdata.supperssion_yratio, &buf[finfo->rom_ois_list[5]], IS_OIS_SUPPERSSION_RATIO_DATA_SIZE);
			memcpy(ois_pinfo->tele_tilt_romdata.cal_mark, &buf[finfo->rom_ois_list[6]], IS_OIS_CAL_MARK_DATA_SIZE);
		}
	}
#endif
...
int is_sec_sensorid_find(struct is_core *core)
{
	...
	} else if (is_sec_fw_module_compare(finfo->header_ver, FW_HM3_L)) {
		specific->rear_sensor_id = SENSOR_NAME_S5KHM3;
