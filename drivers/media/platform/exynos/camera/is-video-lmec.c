...
const struct v4l2_file_operations is_lmec_video_fops;
const struct v4l2_ioctl_ops is_lmec_video_ioctl_ops;
const struct vb2_ops is_lmec_qops;
...
static int is_lmec_video_open(struct file *file)
{
	int ret = 0;
	int ret_err = 0;
	struct is_video *video;
	...
	snprintf(name, sizeof(name), "LMEC");
	ret = open_vctx(file, video, &vctx, device->instance, ENTRY_LMEC, name);
	if (ret) {
...
static int is_lmec_video_close(struct file *file)
{
	int ret = 0;
	int refcount;
	struct is_video_ctx *vctx;
	...
	minfo("[LMEC:V] %s(%d,%d):%d\n", device, __func__, atomic_read(&device->open_cnt), refcount, ret);

	return ret;
}

static unsigned int is_lmec_video_poll(struct file *file,
...
static int is_lmec_video_mmap(struct file *file,
...
const struct v4l2_file_operations is_lmec_video_fops = {
	.owner		= THIS_MODULE,
	.open		= is_lmec_video_open,
	.release	= is_lmec_video_close,
	.poll		= is_lmec_video_poll,
	.unlocked_ioctl	= video_ioctl2,
	.mmap		= is_lmec_video_mmap,
};
...
static int is_lmec_video_querycap(struct file *file, void *fh,
...
static int is_lmec_video_get_format_mplane(struct file *file, void *fh,
...
static int is_lmec_video_set_format_mplane(struct file *file, void *fh,
	struct v4l2_format *format)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_lmec_video_try_format_mplane(struct file *file, void *fh,
	struct v4l2_format *format)
{
	dbg("%s\n", __func__);
	return 0;
...
static int is_lmec_video_reqbufs(struct file *file, void *priv,
...
static int is_lmec_video_querybuf(struct file *file, void *priv,
...
static int is_lmec_video_qbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_lmec_video_dqbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_lmec_video_prepare(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_lmec_video_streamon(struct file *file, void *priv,
...
static int is_lmec_video_streamoff(struct file *file, void *priv,
	enum v4l2_buf_type type)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_lmec_video_enum_input(struct file *file, void *priv,
...
static int is_lmec_video_g_input(struct file *file, void *priv,
	unsigned int *input)
{
	dbg("%s\n", __func__);
	return 0;
...
static int is_lmec_video_s_input(struct file *file, void *priv,
	unsigned int input)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_lmec_video_g_ctrl(struct file *file, void *priv,
...
static int is_lmec_video_g_ext_ctrl(struct file *file, void *priv,
	struct v4l2_ext_controls *ctrls)
{
	return 0;
}

static int is_lmec_video_s_ctrl(struct file *file, void *priv,
	struct v4l2_control *ctrl)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
const struct v4l2_ioctl_ops is_lmec_video_ioctl_ops = {
	.vidioc_querycap		= is_lmec_video_querycap,
	.vidioc_g_fmt_vid_cap_mplane	= is_lmec_video_get_format_mplane,
	.vidioc_s_fmt_vid_cap_mplane	= is_lmec_video_set_format_mplane,
	.vidioc_try_fmt_vid_cap_mplane	= is_lmec_video_try_format_mplane,
	.vidioc_reqbufs			= is_lmec_video_reqbufs,
	.vidioc_querybuf		= is_lmec_video_querybuf,
	.vidioc_qbuf			= is_lmec_video_qbuf,
	.vidioc_dqbuf			= is_lmec_video_dqbuf,
	.vidioc_prepare_buf		= is_lmec_video_prepare,
	.vidioc_streamon		= is_lmec_video_streamon,
	.vidioc_streamoff		= is_lmec_video_streamoff,
	.vidioc_enum_input		= is_lmec_video_enum_input,
	.vidioc_g_input			= is_lmec_video_g_input,
	.vidioc_s_input			= is_lmec_video_s_input,
	.vidioc_g_ctrl			= is_lmec_video_g_ctrl,
	.vidioc_s_ctrl			= is_lmec_video_s_ctrl,
	.vidioc_g_ext_ctrls		= is_lmec_video_g_ext_ctrl,
};

static int is_lmec_queue_setup(struct vb2_queue *vbq,
...
static int is_lmec_start_streaming(struct vb2_queue *vbq,
	unsigned int count)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static void is_lmec_stop_streaming(struct vb2_queue *vbq)
{
	int ret = 0;
	struct is_video_ctx *vctx;
	struct is_queue *queue;
...
static void is_lmec_buffer_queue(struct vb2_buffer *vb)
{
	int ret = 0;
	struct is_video_ctx *vctx;
	struct is_queue *queue;
...
static void is_lmec_buffer_finish(struct vb2_buffer *vb)
{
	int ret;
	struct is_video_ctx *vctx;
	struct is_device_ischain *device;
...
const struct vb2_ops is_lmec_qops = {
	.queue_setup		= is_lmec_queue_setup,
	.buf_init		= is_queue_buffer_init,
	.buf_cleanup		= is_queue_buffer_cleanup,
	.buf_prepare		= is_queue_buffer_prepare,
	.buf_queue		= is_lmec_buffer_queue,
	.buf_finish		= is_lmec_buffer_finish,
	.wait_prepare		= is_queue_wait_prepare,
	.wait_finish		= is_queue_wait_finish,
	.start_streaming	= is_lmec_start_streaming,
	.stop_streaming		= is_lmec_stop_streaming,
};
