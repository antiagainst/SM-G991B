...
int sensor_2l3_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	dbg_sensor(1, "[MOD:D:%d] %s\n", cis->id, __func__);
	is_vendor_set_mipi_clock(subdev);
	...
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		cis->mipi_sensor_mode = sensor_2l3_setfile_A_mipi_sensor_mode;
		cis->mipi_sensor_mode_size = ARRAY_SIZE(sensor_2l3_setfile_A_mipi_sensor_mode);
