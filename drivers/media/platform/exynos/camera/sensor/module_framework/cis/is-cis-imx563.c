...
#else
		/* PD TAIL ON */
		ret |= is_sensor_write8(cis->client, 0x3081, 0x01);
#endif
	}
...
#endif
			ret |= is_sensor_write8(cis->client, 0x3100, shift_count);
			ret |= is_sensor_write8(cis->client, 0x3101, shift_count);
		}
	} else {
		cit_lshift_val = 0;
		ret |= is_sensor_write8(cis->client, 0x3100, 0x00);
		ret = is_sensor_write8(cis->client, 0x3101, 0x00);
	}
...
int sensor_imx563_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (sensor_imx563_cis_is_wdr_mode_on(cis_data)) {
		is_sensor_write8(cis->client, 0x0220, 0x01);
	} else {
	 ...
	 if (sensor_imx563_cis_is_wdr_mode_on(cis_data)) {
		/* Short exposure */
		ret = is_sensor_write16(client, 0x0224, short_coarse_int);
		if (ret < 0)
			goto p_err_i2c_unlock;
		/* Long exposure */
		ret = is_sensor_write16(client, 0x0202, long_coarse_int);
		if (ret < 0)
			goto p_err_i2c_unlock;
	} else {
		/* Short exposure */
	...
	/* CIT shifter */
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = is_sensor_write8(client, 0x3100, coarse_integration_time_shifter);
		if (ret < 0)
			goto p_err_i2c_unlock;
	}
...
int sensor_imx563_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = is_sensor_write8(client, 0x3101, frame_length_lines_shifter);
		if (ret < 0)
			goto p_err_i2c_unlock;
	}
...
static int cis_imx563_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	for (i = 0; i < sensor_id_len; i++) {
		...
		sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_IMX563);
		if (!sensor_peri) {
