...
#define ENABLE_LOGICAL_VIDEO_NODE
...
#define CHECK_NEED_KVADDR_ID(vid)	((0)					\
				...
				|| (debug_lvn && (vid) == IS_VIDEO_IMM_NUM)	\
				)
...
#define CHECK_PARTIAL_PARAM_UPDATE(gid, sensor)	\
		((gid >= GROUP_ID_SS0) && (sensor->cfg->special_mode == IS_SPECIAL_MODE_REMOSAIC))
#define CHECK_PARAM_UP_SKIP_GRP(gid)	\
		((gid == GROUP_ID_ISP0) || (gid == GROUP_ID_MCS0) || \
		(gid == GROUP_ID_YPP) || (gid == GROUP_ID_LME0))
