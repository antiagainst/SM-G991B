...
static int sec_ts_enter_fw_mode(struct sec_ts_data *ts)
{
	...
	sec_ts_delay(20);
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: write fail, enter_fw_mode\n", __func__);
		return 0;
	}

	dev_info(&ts->client->dev, "%s: write ok, enter_fw_mode - 0x%x 0x%x 0x%x\n",
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: read fail, read_boot_status\n", __func__);
		return 0;
	}
	if (fw_status != SEC_TS_STATUS_BOOT_MODE) {
		dev_err(&ts->client->dev, "%s: enter fail! read_boot_status = 0x%x\n", __func__, fw_status);
		return 0;
	}

	dev_info(&ts->client->dev, "%s: Success! read_boot_status = 0x%x\n", __func__, fw_status);

	sec_ts_delay(10);

	ret = ts->sec_ts_i2c_read(ts, SEC_TS_READ_ID, id, 3);
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: read id fail\n", __func__);
		return 0;
	}
	...
	dev_info(&ts->client->dev, "%s: read_boot_id = %02X%02X%02X\n", __func__, id[0], id[1], id[2]);
...
static int sec_ts_sw_reset(struct sec_ts_data *ts)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: write fail, sw_reset\n", __func__);
		return 0;
	}
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: time out\n", __func__);
		return 0;
	}

	dev_info(&ts->client->dev, "%s: sw_reset\n", __func__);

	/* Sense_on */
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_SENSE_ON, NULL, 0);
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: write fail, Sense_on\n", __func__);
		return 0;
	}
...
static void sec_ts_save_version_of_bin(struct sec_ts_data *ts, const fw_header *fw_hd)
{
	...
	dev_info(&ts->client->dev, "%s: img_ver of bin = %x.%x.%x.%x\n", __func__,
	...
	dev_info(&ts->client->dev, "%s: core_ver of bin = %x.%x.%x.%x\n", __func__,
	...
	dev_info(&ts->client->dev, "%s: config_ver of bin = %x.%x.%x.%x\n", __func__,
...
static int sec_ts_save_version_of_ic(struct sec_ts_data *ts)
{
	...
	dev_info(&ts->client->dev, "%s: IC Image version info : %x.%x.%x.%x\n",
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: core version read error\n", __func__);
		return -EIO;
	}
	dev_info(&ts->client->dev, "%s: IC Core version info : %x.%x.%x.%x,\n",
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: config version read error\n", __func__);
		return -EIO;
	}
	dev_info(&ts->client->dev, "%s: IC config version info : %x.%x.%x.%x\n",
...
static int sec_ts_check_firmware_version(struct sec_ts_data *ts, const u8 *fw_info)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: fail to read ic version\n", __func__);
		return -EIO;
	}
	...
	for (i = 0; i < 2; i++) {
		if (ts->plat_data->img_version_of_ic[i] != ts->plat_data->img_version_of_bin[i]) {
			dev_err(&ts->client->dev, "%s: do not matched version info\n", __func__);
			return 0;
		}
	}
...
static int sec_ts_flashwrite(struct sec_ts_data *ts, u32 mem_addr, u8 *mem_data, u32 mem_size, int retry)
{
	...
	for (page_idx = page_num - 1;; page_idx--) {
		...
		if (ts->boot_ver[0] == 0xB2) {
			...
			if (ret < 0) {
				dev_err(&ts->client->dev, "%s: fw write failed, page_idx = %u\n", __func__, page_idx);
				goto err;
			}
...
static int sec_ts_memoryblockread(struct sec_ts_data *ts, u32 mem_addr, int mem_size, u8 *buf)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: send command failed, %02X\n", __func__, cmd[0]);
		return -EIO;
	}
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: memory read failed\n", __func__);
		return -EIO;
	}
/*
...
static int sec_ts_chunk_update(struct sec_ts_data *ts, u32 addr, u32 size, u8 *data, int retry)
{
	...
	if (write_size != fw_size) {
		dev_err(&ts->client->dev, "%s: fw write failed\n", __func__);
		ret = -1;
		goto err_write_fail;
	}
	...
	if (!mem_rb) {
		dev_err(&ts->client->dev, "%s: vzalloc failed\n", __func__);
		ret = -1;
		goto err_write_fail;
	}
	...
	if (sec_ts_memoryread(ts, addr, mem_rb, fw_size) >= 0) {
		...
		if (fw_size != ii) {
			dev_err(&ts->client->dev, "%s: fw verify fail\n", __func__);
			ret = -1;
			goto out;
		}
	} else {
	...
	dev_info(&ts->client->dev, "%s: verify done(%d)\n", __func__, ret);
...
static int sec_ts_firmware_update(struct sec_ts_data *ts, const u8 *data, size_t size, int bl_update, int restore_cal, int retry)
{
	...
	if (!sec_ts_enter_fw_mode(ts)) {
		dev_err(&ts->client->dev, "%s: firmware mode failed\n", __func__);
		return -1;
	}

	if (bl_update && (ts->boot_ver[0] == 0xB4)) {
		dev_info(&ts->client->dev, "%s: bootloader is up to date\n", __func__);
		return 0;
	}

	dev_info(&ts->client->dev, "%s: firmware update retry :%d\n", __func__, retry);

	fw_hd = (fw_header *)fd;
	fd += sizeof(fw_header);

	if (fw_hd->signature != SEC_TS_FW_HEADER_SIGN) {
		dev_err(&ts->client->dev, "%s: firmware header error = %08X\n", __func__, fw_hd->signature);
		return -1;
	}

	dev_err(&ts->client->dev, "%s: num_chunk : %d\n", __func__, fw_hd->num_chunk);

	for (i = 0; i < fw_hd->num_chunk; i++) {
		fw_ch = (fw_chunk *)fd;

		dev_err(&ts->client->dev, "%s: [%d] 0x%08X, 0x%08X, 0x%08X, 0x%08X\n", __func__, i,
				fw_ch->signature, fw_ch->addr, fw_ch->size, fw_ch->reserved);

		if (fw_ch->signature != SEC_TS_FW_CHUNK_SIGN) {
			dev_err(&ts->client->dev, "%s: firmware chunk error = %08X\n", __func__, fw_ch->signature);
			return -1;
		}
		fd += sizeof(fw_chunk);
		ret = sec_ts_chunk_update(ts, fw_ch->addr, fw_ch->size, fd, retry);
		if (ret < 0) {
			dev_err(&ts->client->dev, "%s: firmware chunk write failed, addr=%08X, size = %d\n", __func__, fw_ch->addr, fw_ch->size);
			return -1;
		}
		fd += fw_ch->size;
	}
	...
	dev_info(&ts->client->dev, "%s: cal_count(0x%02X) pat_function dt(%d) restore_cal(%d) magic_cal(%d)\n",
	...
	if (!bl_update) {
		...
		if ((ts->cal_count == 0) || (ts->cal_count == 0xFF) || (magic_cal == true)) {
			dev_err(&ts->client->dev, "%s: RUN OFFSET CALIBRATION(0x%02X)\n", __func__, ts->cal_count);

			ret = sec_ts_execute_force_calibration(ts, OFFSET_CAL_SEC);
			if (ret < 0)
				dev_err(&ts->client->dev, "%s: fail to write OFFSET CAL SEC!\n", __func__);
			...
			if (ret >= 0 && magic_cal) {
				...
				sec_ts_delay(20);
				ts->cal_count = get_tsp_nvm_data(ts, SEC_TS_NVM_OFFSET_CAL_COUNT);
				dev_info(&ts->client->dev, "%s: cal_count = [%02X]\n",
							__func__, ts->cal_count);
			}
			...
			if (ret < 0) {
				...
				dev_info(&ts->client->dev, "%s: tune_fix_ver [%02X %02X]\n", __func__, buff[0], buff[1]);
			}
		} else {
			dev_err(&ts->client->dev, "%s: DO NOT CALIBRATION(0x%02X)\n", __func__, ts->cal_count);
		}
#else
		/* always calibration after fw update */
		dev_err(&ts->client->dev, "%s: RUN OFFSET CALIBRATION\n", __func__);
		...
		if (ts->sec_ts_i2c_read(ts, SEC_TS_READ_BOOT_STATUS, &fw_status, 1) < 0) {
			dev_err(&ts->client->dev, "%s: read fail, read_boot_status = 0x%x\n", __func__, fw_status);
			return -EIO;
		}

		if (fw_status != SEC_TS_STATUS_APP_MODE) {
			dev_err(&ts->client->dev, "%s: fw update sequence done, BUT read_boot_status = 0x%x\n", __func__, fw_status);
			return -EIO;
		}

		dev_info(&ts->client->dev, "%s: fw update Success! read_boot_status = 0x%x\n", __func__, fw_status);

		return 1;
	} else {

		if (ts->sec_ts_i2c_read(ts, SEC_TS_READ_ID, tBuff, 3) < 0) {
			dev_err(&ts->client->dev, "%s: read device id fail after bl fw download\n", __func__);
			return -EIO;
		}

		if (tBuff[0] == 0xA0) {
			dev_info(&ts->client->dev, "%s: bl fw download success - device id = %02X\n", __func__, tBuff[0]);
			return -EIO;
		} else {
			dev_err(&ts->client->dev, "%s: bl fw id does not match - device id = %02X\n", __func__, tBuff[0]);
			return -EIO;
		}
	}
...
int sec_ts_firmware_update_bl(struct sec_ts_data *ts)
{
	...
	dev_info(&ts->client->dev, "%s: initial bl update %s\n", __func__, fw_path);

	/* Loading Firmware------------------------------------------ */
	if (request_firmware(&fw_entry, fw_path, &ts->client->dev) !=  0) {
		dev_err(&ts->client->dev, "%s: bt is not available\n", __func__);
		goto err_request_fw;
	}
	dev_info(&ts->client->dev, "%s: request bt done! size = %d\n", __func__, (int)fw_entry->size);
...
int sec_ts_bl_update(struct sec_ts_data *ts)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: bl update command send fail!\n", __func__);
		goto err;
	}
	sec_ts_delay(10);
	...
	do {
		...
		if (ret < 0) {
			dev_err(&ts->client->dev, "%s: read bl update status fail!\n", __func__);
			goto err;
		}
		sec_ts_delay(2);
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: write passwd fail!\n", __func__);
		goto err;
	}
	...
	if (tBuff[0]  == 0xB4) {
		dev_info(&ts->client->dev, "%s: bl update completed!\n", __func__);
		ret = 1;
	} else {
		dev_info(&ts->client->dev, "%s: bl updated but bl version not matching, ver=%02X\n", __func__, tBuff[0]);
		goto err;
	}
...
int sec_ts_firmware_update_on_probe(struct sec_ts_data *ts, bool force_update)
{
	...
	if (ts->plat_data->bringup == 1) {
		dev_err(&ts->client->dev, "%s: bringup. do not update\n", __func__);
		return 0;
	}
	...
	dev_info(&ts->client->dev, "%s: initial firmware update %s, cal:%X\n",
					__func__, fw_path, ts->cal_status);

	/* Loading Firmware */
	if (request_firmware(&fw_entry, fw_path, &ts->client->dev) !=  0) {
		dev_err(&ts->client->dev, "%s: firmware is not available\n", __func__);
		goto err_request_fw;
	}
	dev_info(&ts->client->dev, "%s: request firmware done! size = %d\n", __func__, (int)fw_entry->size);
	...
	if ((result <= 0) && (!force_update)) {
		...
		if (ts->plat_data->pat_function == PAT_CONTROL_FORCE_UPDATE) {
			dev_info(&ts->client->dev, "%s: run forced f/w update and excute autotune\n", __func__);
		} else {
			dev_info(&ts->client->dev, "%s: skip - fw update & nv read\n", __func__);
			goto err_request_fw;
		}
	}
	...
	if (ts->cal_count == 0xFF) {
		...
		set_tsp_nvm_data_clear(ts, SEC_TS_NVM_OFFSET_TUNE_VERSION);
		set_tsp_nvm_data_clear(ts, SEC_TS_NVM_OFFSET_TUNE_VERSION+1);
		dev_info(&ts->client->dev, "%s: initialize nv as default value & excute autotune\n", __func__);
	}
	...
	dev_info(&ts->client->dev, "%s: tune_fix_ver [%04X] afe_base [%04X]\n", __func__, ts->tune_fix_ver, ts->plat_data->afe_base);
	...
	/* ic fw ver > bin fw ver && force is false*/
	if ((result <= 0) && (!force_update)) {
		dev_info(&ts->client->dev, "%s: skip fw update\n", __func__);
		goto err_request_fw;
	}
#endif
...
static int sec_ts_load_fw_from_bin(struct sec_ts_data *ts)
{
	...
	dev_info(&ts->client->dev, "%s: initial firmware update  %s\n", __func__, fw_path);
...
static int sec_ts_load_fw_from_ums(struct sec_ts_data *ts)
{
	...
	if (IS_ERR(fp)) {
		dev_err(ts->dev, "%s: failed to open %s.\n", __func__,
...
*/
			dev_info(&ts->client->dev, "%s: firmware version %08X\n", __func__, fw_hd->fw_ver);
			dev_info(&ts->client->dev, "%s: parameter version %08X\n", __func__, fw_hd->para_ver);
		...
		if (error < 0)
			dev_err(ts->dev, "%s: failed update firmware\n",
					__func__);
...
static int sec_ts_load_fw_from_ffu(struct sec_ts_data *ts)
{
	...
	dev_info(ts->dev, "%s: Load firmware : %s\n", __func__, fw_path);
...
int sec_ts_firmware_update_on_hidden_menu(struct sec_ts_data *ts, int update_type)
{
	...
	case BL:
		...
		break;
	default:
		dev_err(ts->dev, "%s: Not support command[%d]\n",
