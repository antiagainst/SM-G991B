		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(&ts->client->dev, "%s: %02X%02X%02X%02X\n", __func__,
				    ...
				    const char *buf, size_t count)
{
	...
	dev_info(&ts->client->dev, "%s: clear\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(&ts->client->dev, "%s: touch:%d, force:%d, aod:%d, spay:%d\n", __func__,
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(&ts->client->dev, "%s: max:%d, min:%d, avg:%d\n", __func__,
...
static void fw_update(void *device_data)
{
	...
	if (ts->power_status == SEC_TS_STATE_POWER_OFF) {
		dev_err(&ts->client->dev, "%s: [ERROR] Touch is stopped\n",
				__func__);
	...
	if (retval < 0) {
		...
		dev_info(&ts->client->dev, "%s: success [%d]\n", __func__, retval);
	}
}
		...
		short *max)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: Set rawdata type failed\n", __func__);
		goto ErrorExit;
	}
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: read rawdata failed!\n", __func__);
		goto ErrorRelease;
	}
...
#ifdef DEBUG_MSG
	dev_info(&ts->client->dev, "%s: 02X%02X%02X readbytes=%d\n", __func__,
 ...
 */
	for (ii = 0; ii < data_length; ii += 2) {
		...
		*max = max(*max, ts->pFrame[jj]);
		...
		dev_info(&ts->client->dev, "%s: [%s][%d] %d\n", __func__,
		...
		struct sec_cmd_data *sec, struct sec_ts_test_mode *mode)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: failed to read frame\n",
				__func__);
		goto error_test_fail;
	}
...
static void get_fw_ver_ic(void *device_data)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: firmware version read error\n", __func__);
		snprintf(buff, sizeof(buff), "%s", "NG");
		sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		return;
...
static void get_threshold(void *device_data)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: threshold write type failed. ret: %d\n", __func__, ret);
		snprintf(buff, sizeof(buff), "%s", "NG");
		goto err;
	}
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: read threshold fail!\n", __func__);
		snprintf(buff, sizeof(buff), "%s", "NG");
		goto err;
	}

	dev_info(&ts->client->dev, "0x%02X, 0x%02X\n",
...
static void set_mis_cal_spec(void *device_data)
{
	...
	if (ts->plat_data->mis_cal_check == 0) {
		dev_err(&ts->client->dev, "%s: [ERROR] not support, %d\n", __func__);
		goto NG;
		...
		if ((sec->cmd_param[0] < 0 || sec->cmd_param[0] > 255) ||
			...
			if (ret < 0) {
				dev_err(&ts->client->dev, "%s: nvm write failed. ret: %d\n", __func__, ret);
				goto NG;
			} else {
				dev_info(&ts->client->dev, "%s: tx gap=%d, rx gap=%d, peak=%d\n", __func__, wreg[0], wreg[1], wreg[2]);
				sec_ts_delay(20);
			}
		}
	}
...
static void get_mis_cal_info(void *device_data)
{
	...
	if (ts->plat_data->mis_cal_check == 0) {
		...
		if (ret < 0) {
			dev_err(&ts->client->dev, "%s: i2c fail!, %d\n", __func__, ret);
			mis_cal_data = 0xF3;
			goto NG;
		} else {
			dev_info(&ts->client->dev, "%s: miss cal data : %d\n", __func__, mis_cal_data);
		}
		...
		if (ret < 0) {
			...
			dev_info(&ts->client->dev, "%s: miss cal spec : %d,%d,%d\n", __func__,
...
static void get_checksum_data(void *device_data)
{
	...
	if (ret < 0) {
		enable_irq(ts->client->irq);
		dev_err(&ts->client->dev, "%s: boot complete failed\n", __func__);
		snprintf(buff, sizeof(buff), "%s", "NG");
		goto err;
	}
	...
	if (ret < 0 || (data[0] != 0x80)) {
		enable_irq(ts->client->irq);
		dev_err(&ts->client->dev, "%s: firmware integrity failed, ret:%d, data:%X\n",
				__func__, ret, data[0]);
		snprintf(buff, sizeof(buff), "%s", "NG");
		goto err;
	}
	...
	if (ret < 0 || (data[1] != SEC_TS_STATUS_APP_MODE)) {
		enable_irq(ts->client->irq);
		dev_err(&ts->client->dev, "%s: boot status failed, ret:%d, data:%X\n", __func__, ret, data[0]);
		snprintf(buff, sizeof(buff), "%s", "NG");
		goto err;
	}
	...
	if (ret < 0 || (data[3] == TOUCH_SYSTEM_MODE_FLASH)) {
		enable_irq(ts->client->irq);
		dev_err(&ts->client->dev, "%s: touch status failed, ret:%d, data:%X\n", __func__, ret, data[3]);
		snprintf(buff, sizeof(buff), "%s", "NG");
		goto err;
	}
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: read get_checksum result fail!\n", __func__);
	...
	dev_info(&ts->client->dev, "%s: checksum = %02X\n", __func__, csum);
...
void sec_ts_run_rawdata_all(struct sec_ts_data *ts)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: 19,Offset error ## ret:%d\n", __func__, ret);
	} else {
		dev_err(&ts->client->dev, "%s: 19,Offset Max/Min %d,%d ##\n", __func__, max, min);
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: 0,Ambient error ## ret:%d\n", __func__, ret);
	} else {
		dev_err(&ts->client->dev, "%s: 0,Ambient Max/Min %d,%d ##\n", __func__, max, min);
		sec_ts_release_tmode(ts);
	}
...
int get_tsp_nvm_data(struct sec_ts_data *ts, u8 offset)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: fail to write Sense_off\n", __func__);
		goto out_nvm;
	}

	dev_dbg(&ts->client->dev, "%s: SENSE OFF\n", __func__);
	...
	dev_dbg(&ts->client->dev, "%s: CLEAR EVENT STACK\n", __func__);
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: nvm send command failed. ret: %d\n", __func__, ret);
		goto out_nvm;
	}
	...
	dev_info(&ts->client->dev, "%s: offset:%u  data:%02X\n", __func__,offset, buff[0]);
...
out_nvm:
	...
	dev_dbg(&ts->client->dev, "%s: SENSE ON\n", __func__);
...
int get_tsp_nvm_data_by_size(struct sec_ts_data *ts, u8 offset, int length, u8 *data)
{
	...
	dev_info(&ts->client->dev, "%s: offset:%u, length:%d, size:%d\n", __func__, offset, length, sizeof(data));
...
static void set_tsp_test_result(void *device_data)
{
	...
	dev_info(&ts->client->dev, "%s: %d, %d, %d, %d, 0x%X\n", __func__,
	...
	dev_info(&ts->client->dev, "%s: command (1)%X, (2)%X: %X\n",
...
static void get_tsp_test_result(void *device_data)
{
	...
	dev_info(&ts->client->dev, "%s: [0x%X][0x%X] M:%d, M:%d, A:%d, A:%d\n",
...
static void increase_disassemble_count(void *device_data)
{
	...
	dev_info(&ts->client->dev, "%s: disassemble count is #1 %d\n", __func__, buff[2]);
	...
	dev_info(&ts->client->dev, "%s: check disassemble count: %d\n", __func__, buff[0]);
...
static void get_disassemble_count(void *device_data)
{
	...
	dev_info(&ts->client->dev, "%s: read disassemble count: %d\n", __func__, buff[0]);
...
static void glove_mode(void *device_data)
{
	...
	if (sec->cmd_param[0] < 0 || sec->cmd_param[0] > 1) {
		...
		if (retval < 0) {
			dev_err(&ts->client->dev, "%s: failed, retval = %d\n", __func__, retval);
			snprintf(buff, sizeof(buff), "NG");
			sec->cmd_state = SEC_CMD_STATUS_FAIL;
		} else {
			snprintf(buff, sizeof(buff), "OK");
...
static void clear_cover_mode(void *device_data)
{
	...
	dev_info(&ts->client->dev, "%s: start clear_cover_mode %s\n", __func__, buff);
	sec_cmd_set_default_result(sec);
...
int execute_selftest(struct sec_ts_data *ts, bool save_result)
{
	...
	dev_info(&ts->client->dev, "%s: Self test start!\n", __func__);
	rc = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_SELFTEST, tpara, 2);
	if (rc < 0) {
		dev_err(&ts->client->dev, "%s: Send selftest cmd failed!\n", __func__);
		goto err_exit;
	}
	...
	if (rc < 0) {
		dev_err(&ts->client->dev, "%s: Selftest execution time out!\n", __func__);
		goto err_exit;
	}

	dev_info(&ts->client->dev, "%s: Self test done!\n", __func__);
...
static void run_trx_short_test(void *device_data)
{
	...
	if (ts->power_status == SEC_TS_STATE_POWER_OFF) {
		dev_err(&ts->client->dev, "%s: Touch is stopped!\n", __func__);
		snprintf(buff, sizeof(buff), "%s", "TSP turned off");
		sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
		sec->cmd_state = SEC_CMD_STATUS_NOT_APPLICABLE;
		return;
...
int sec_ts_execute_force_calibration(struct sec_ts_data *ts, int cal_mode)
{
	...
	if (ts->sec_ts_i2c_write(ts, cmd, NULL, 0) < 0) {
		dev_err(&ts->client->dev, "%s: Write Cal commend failed!\n", __func__);
		return rc;
	}
...
static void run_force_calibration(void *device_data)
{
	...
	if (rc < 0) {
		...
		if (rc < 0)
			dev_err(&ts->client->dev, "%s: fail to write PRESSURE CAL!\n", __func__);
#endif
		...
		if (ts->plat_data->mis_cal_check) {
			...
			dev_err(&ts->client->dev, "%s: try mis Cal. check\n", __func__);
		...
		buff[1] = 0;
		buff[2] = ts->cal_count;
		dev_info(&ts->client->dev, "%s: write to nvm cal_count(%2X)\n",
					__func__, buff[2]);
		...
		if (rc < 0) {
			dev_err(&ts->client->dev, "%s: Image version read error\n", __func__);
		} else {
			memset(buff, 0x00, SEC_CMD_STR_LEN);
			...
			buff[2] = img_ver[2];
			buff[3] = img_ver[3];
			dev_info(&ts->client->dev, "%s: write tune_ver to nvm (%2X %2X)\n", __func__, buff[2], buff[3]);
...
static void run_force_pressure_calibration(void *device_data)
{
	...
	dev_info(&ts->client->dev, "%s: count:%d\n", __func__, ts->pressure_cal_base);
...
static int read_pressure_data(struct sec_ts_data *ts, u8 type, short *value)
{
	...
	if (data[0] != type) {
		dev_info(&ts->client->dev, "%s: type change to %02X\n", __func__, type);
	...
	dev_info(&ts->client->dev, "%s: Left: %d, Center: %d, Rignt: %d\n",
...
static void set_pressure_strength(void *device_data)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: cmd write failed. ret: %d\n", __func__, ret);
		goto err_comm_str;
	}
...
err_cmd_param_str:
	dev_info(&ts->client->dev, "%s: parameter error: %u\n",
...
static void set_pressure_rawdata(void *device_data)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: nvm read failed. ret: %d\n", __func__, ret);
		goto err_comm_raw;
	}
...
static void set_pressure_data_index(void *device_data)
{
	...
	if (sec->cmd_param[0] == 0) {
		dev_info(&ts->client->dev, "%s: clear calibration result\n", __func__);
...
static void set_pressure_strength_clear(void *device_data)
{
	...
	if (!data) {
		dev_err(&ts->client->dev, "%s failed to allocate memory. ret: %d\n", __func__, ret);
		goto err_comm_str;
	}
...
static void set_pressure_user_level(void *device_data)
{
	...
	dev_info(&ts->client->dev, "%s: set user level: %d\n", __func__, data[0]);
	...
	dev_info(&ts->client->dev, "%s: HIGH THD: %d\n", __func__, data[0]);
	...
	dev_info(&ts->client->dev, "%s: LOW THD: %d\n", __func__, data[0]);
...
static void get_pressure_user_level(void *device_data)
{
	...
	dev_err(&ts->client->dev, "%s: set user level: %d\n", __func__, data[0]);
	ts->pressure_user_level = data[0];
...
static void set_wirelesscharger_mode(void *device_data)
{
	...
	if (ts->power_status == SEC_TS_STATE_POWER_OFF) {
		dev_err(&ts->client->dev, "%s: fail to enable w-charger status, POWER_STATUS=OFF\n", __func__);
		goto NG;
	}
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: Failed to send command 74\n", __func__);
		goto NG;
	}

	dev_err(&ts->client->dev, "%s: %s, status =%x\n",
...
static void set_aod_rect(void *device_data)
{
	...
	dev_info(&ts->client->dev, "%s: w:%d, h:%d, x:%d, y:%d\n",
...
static void get_aod_rect(void *device_data)
{
	...
	if (ts->use_sponge) {
		...
		if (ret < 0) {
			dev_err(&ts->client->dev, "%s: Failed to read rect\n", __func__);
			goto NG;
		}
		enable_irq(ts->client->irq);
	}
...
void set_grip_data_to_ic(struct sec_ts_data *ts, u8 flag)
{
	...
	dev_info(&ts->client->dev, "%s: flag: %02X (clr,lan,nor,edg,han)\n", __func__, flag);
	...
	if (flag & G_SET_EDGE_HANDLER) {
		...
		dev_info(&ts->client->dev, "%s: 0x%02X %02X,%02X,%02X,%02X\n",
	...
	if (flag & G_SET_EDGE_ZONE) {
		...
		dev_info(&ts->client->dev, "%s: 0x%02X %02X,%02X\n",
	...
	if (flag & G_CLR_LANDSCAPE_MODE) {
		...
		dev_info(&ts->client->dev, "%s: 0x%02X %02X\n",
...
static void set_grip_data(void *device_data)
{
	...
	if (sec->cmd_param[0] == 0) {	// edge handler
		if (sec->cmd_param[1] == 0) {	// clear
			...
			dev_err(&ts->client->dev, "%s: cmd1 is abnormal, %d (%d)\n",
		...
		dev_err(&ts->client->dev, "%s: cmd0 is abnormal, %d", __func__, sec->cmd_param[0]);
		goto err_grip_data;
	}
...
static void dex_enable(void *device_data)
{
	...
	if (!ts->plat_data->support_dex) {
		dev_err(&ts->client->dev, "%s: not support DeX mode\n", __func__);
		goto NG;
	}

	if ((sec->cmd_param[0] < 0 || sec->cmd_param[0] > 1) &&
		(sec->cmd_param[1] < 0 || sec->cmd_param[1] > 1)) {
		dev_err(&ts->client->dev, "%s: not support param\n", __func__);
		goto NG;
	}

	ts->dex_mode = sec->cmd_param[0];
	if (ts->dex_mode) {
		dev_err(&ts->client->dev, "%s: set DeX touch_pad mode%s\n",
		...
		dev_err(&ts->client->dev, "%s: set touch mode\n", __func__);
...
static void set_log_level(void *device_data)
{
	...
	if ((sec->cmd_param[0] < 0 || sec->cmd_param[0] > 1) ||
		...
		dev_err(&ts->client->dev, "%s: para out of range\n", __func__);
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: Read Event type enable status fail\n", __func__);
		snprintf(buff, sizeof(buff), "%s", "Read Stat Fail");
		goto err;
	}

	dev_info(&ts->client->dev, "%s: STATUS_EVENT enable = 0x%02X, 0x%02X\n",
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "%s: Write Event type enable status fail\n", __func__);
	...
	if (sec->cmd_param[0] == 1 && sec->cmd_param[1] == 1 && sec->cmd_param[2] == 1  && sec->cmd_param[3] == 1 &&
		...
		if (ret < 0) {
			dev_err(&ts->client->dev, "%s: Write Vendor Event Level fail\n", __func__);
			snprintf(buff, sizeof(buff), "%s", "Write Stat Fail");
			goto err;
		}
	} else {
	...
	dev_info(&ts->client->dev, "%s: ERROR : %d, INFO : %d, USER_INPUT : %d, INFO_SPONGE : %d, VENDOR_INFO : %d, VENDOR_EVENT_LEVEL : %d\n",
