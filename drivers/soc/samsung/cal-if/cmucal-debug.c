...
static unsigned int cmu_core_base = 0x0;
void __iomem *cmu_core;
...
void cmucal_dbg_set_cmu_core_base(u32 base_addr)
{
	cmu_core_base = base_addr + BASE_MOVE;
	pr_info("cmu_core_base : 0x%x\n", base_addr);

	cmu_core = ioremap(cmu_core_base, SZ_4K);
	if (!cmu_core)
		pr_err("%s: cmu_core ioremap failed\n", __func__);
}
EXPORT_SYMBOL_GPL(cmucal_dbg_set_cmu_core_base);
