...
set_fore:
	...
	if (q->desc_ul[last_ptr].last_desc == 0) {
		q->desc_ul[last_ptr].last_desc = 1;
#if IS_ENABLED(CONFIG_EXYNOS_MEMORY_LOGGER)
		pr_memlog("2nd last desc: 0x%08x end bit: %d hw set: %d fore desc: 0x%08x\n",
				last_ptr,
				q->desc_ul[last_ptr].last_desc,
				q->desc_ul[last_ptr].hw_set,
				fore_ptr);
#endif
	}
