...
static struct adv_tracer_info *exynos_adv_tracer = NULL;
	...
	switch(cmd->cmd_raw.cmd) {
		...
		case EAT_IPC_CMD_EXCEPTION_DBGC:
			dev_err(adv_tracer_ipc->dev,
					"DBGC occurred exception! "
					"(SP: 0x%x, LR:0x%x, PC:0x%x)\n",
...
static int adv_tracer_ipc_init(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "pmu_dbgcore_config",
			&pmu_dbgcore_config);
	if (ret)
		dev_err(&pdev->dev, "pmu_dbgcore_config is no data\n");
	ret = of_property_read_u32(node, "pmu_dbgcore_status",
			&pmu_dbgcore_status);
	if (ret)
