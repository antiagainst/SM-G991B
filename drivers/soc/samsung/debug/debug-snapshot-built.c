...
static hook_func_t ref_hook_func;
...
void dbg_snapshot_work(void *worker, void *v_task, void *fn, int en)
{
	...
	orig_dss_log->work[cpu][i].fn = (work_func_t)fn;
	orig_dss_log->work[cpu][i].en = en;
}

void dbg_snapshot_cpuidle(char *modes, unsigned state, s64 diff, int en)
{
	int cpu = raw_smp_processor_id();
	unsigned long i;
...
void dbg_snapshot_irq(int irq, void *fn, void *val, unsigned long long start_time, int en)
{
	...
	orig_dss_log->irq[cpu][i].fn = (void *)fn;
...
void dbg_snapshot_spinlock(void *v_lock, int en)
{
	...
	if (unlikely(!dbg_snapshot_is_log_item_enabled(DSS_LOG_SPINLOCK_ID)))
		return;

	i = atomic_fetch_inc(&orig_dss_log_misc->spinlock_log_idx[cpu]) &
		(ARRAY_SIZE(orig_dss_log->spinlock[0]) - 1);
	orig_dss_log->spinlock[cpu][i].time = cpu_clock(cpu);
	orig_dss_log->spinlock[cpu][i].jiffies = jiffies_64;
	orig_dss_log->spinlock[cpu][i].lock = lock;
	orig_dss_log->spinlock[cpu][i].locked = lock->raw_lock.locked;
	orig_dss_log->spinlock[cpu][i].pending = lock->raw_lock.pending;
	orig_dss_log->spinlock[cpu][i].tail = lock->raw_lock.tail;
	orig_dss_log->spinlock[cpu][i].en = en;

	for (j = 0; j < DSS_CALLSTACK_MAX_NUM; j++)
		orig_dss_log->spinlock[cpu][i].caller[j] = return_address(j + 1);
}
...
static int dbg_snapshot_built_probe(struct platform_device *pdev)
{
	...
	if (!rmem) {
		dev_err(&pdev->dev, "no such reseved mem of node name %s\n",
	...
	ref_hook_func = orig_dss_base->param->hook_func;

	register_hook_logbuf(ref_hook_func);
	register_hook_logger(ref_hook_func);
...
static const struct of_device_id dss_built_of_match[] = {
	{ .compatible	= "debug-snapshot,built" },
	{},
};
...
MODULE_DESCRIPTION("Debug-Snapshot-built");
