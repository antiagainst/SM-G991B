		...
		struct device_attribute *attr, const char *buf, size_t n)
{
	...
	/* Disable U1/U2 & get speed */
	{
		...
		/* Disable U1/U2 */
		reg_addr = ioremap(0x10E0c700, 0x10);
		reg = readl(reg_addr + 0x4); /* DCTL register */
		reg &= ~(BIT(9) | BIT(10) | BIT(11) | BIT(12));
		writel(reg, reg_addr + 0x4);

		/* Read Connect Speed Value. */
		speed_val = readl(reg_addr + 0xc) & 0x7; /* DSTS register */
		iounmap(reg_addr);
	}
...
static void exynos_usbdrd_pipe3_init(struct exynos_usbdrd_phy *phy_drd)
{
	...
	ret = phy_exynos_usbdp_g2_v4_enable(&phy_drd->usbphy_sub_info);
}
