...
#else
	if (queue)
		queue_work_on(7, g_rndis_mp->g_uether_wq, &g_rndis_mp->rx_work);
#endif
}
...
static int tx_task(struct eth_dev *dev, struct usb_request *req)
{
	...
	if (gadget_is_dualspeed(dev->gadget) &&
		...
		g_rndis_mp->tx_qlen++;
		if (g_rndis_mp->tx_qlen == (dev->qmult/2)) {
			req->no_interrupt = 0;
			g_rndis_mp->tx_qlen = 0;
		} else {
			req->no_interrupt = 1;
		}
	} else {
					...
					struct net_device *net)
{
	...
	/* temporarily stop TX queue when the freelist empties */
	if (list_empty(&dev->tx_reqs) && (g_rndis_mp->tx_skb_hold_count >= (g_rndis_mp->dl_max_pkts_per_xfer -1)))
	...
	if (g_rndis_mp->multi_pkt_xfer) {
		...
		if (g_rndis_mp->tx_skb_hold_count < g_rndis_mp->dl_max_pkts_per_xfer) {
			...
			tx_timeout = g_rndis_mp->occured_timeout ?
						MIN_TX_TIMEOUT_NSECS : MAX_TX_TIMEOUT_NSECS;
			g_rndis_mp->occured_timeout = 0;
...
{
	g_rndis_mp = kmalloc(sizeof(*g_rndis_mp), GFP_KERNEL);
	g_eth_dev = kmalloc(sizeof(*g_eth_dev), GFP_KERNEL);
	g_rndis_mp->g_uether_wq = create_workqueue("uether");
	if (!g_rndis_mp->g_uether_wq) {
...
{
	destroy_workqueue(g_rndis_mp->g_uether_wq);
	kfree(g_rndis_mp);
	kfree(g_eth_dev);
}
