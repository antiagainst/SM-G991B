...
static int _tzdev_mem_register(struct tzdev_mem_reg *mem, sk_pfn_t *pfns,
		unsigned long nr_pages, unsigned int is_writable, unsigned int is_kern)
{
	int ret, id;
	struct tz_iwio_aux_channel *ch;
	...
	ret = tz_format_cred(&mem->cred, is_kern);
	if (ret) {
	...
	mutex_lock(&tzdev_mem_mutex);
	ret = idr_alloc(&tzdev_mem_map, mem, 1, 0, GFP_KERNEL);
	if (ret < 0) {
...
int tzdev_mem_register_user(unsigned long size, unsigned int write)
{
	...
	id = _tzdev_mem_register(mem, pfns, nr_pages, flags, 0);
	if (id < 0) {
		ret = id;
		goto out_mem;
	}
		...
		tzdev_mem_free_func_t free_func, void *free_data)
{
	...
	id = _tzdev_mem_register(mem, pfns, nr_pages, flags, 1);
	if (id < 0) {
		ret = id;
		goto out_mem;
	}
