 ...
 * @cpufreq_cdev:	&struct exynos_cpufreq_cooling_device for this cpu
 * @cpu:	cpu number
 ...
 */
static u32 get_load(struct exynos_cpufreq_cooling_device *cpufreq_cdev, int cpu,
		    int cpu_idx)
{
	u32 load;
 ...
 * @cpufreq_cdev:	&exynos_cpufreq_cooling_device for this cdev
 * @freq:	current frequency
 *
 * Return: the dynamic power consumed by the cpus described by
 * @cpufreq_cdev.
 */
static u32 get_dynamic_power(struct exynos_cpufreq_cooling_device *cpufreq_cdev,
			     unsigned long freq)
{
	u32 raw_cpu_power;
				 ...
				 unsigned long state)
{
	...
	dbg_snapshot_thermal(NULL, 1001, cpufreq_cdev->cdev->type,
...
EXPORT_SYMBOL_GPL(cpu_cooling_notifier_register);

int cpu_cooling_notifier_unregister(struct notifier_block *n)
{
	return blocking_notifier_chain_unregister(&cpu_notifier, n);
}
EXPORT_SYMBOL_GPL(cpu_cooling_notifier_unregister);
			...
			get_static_t plat_static_func, char *tz_name)
{
	...
	parse_ect_cooling_level(cdev, tz_name);
