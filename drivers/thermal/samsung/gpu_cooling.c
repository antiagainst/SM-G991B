 ...
 * otherwise.
 */
u32 gpufreq_cooling_get_freq(unsigned int gpu, unsigned long level)
{
	unsigned int val = 0;
				 ...
				 unsigned long cooling_state)
{
	...
	gpu_tmu_notifier(NULL, GPU_THROTTLING, &gpu_cooling_freq);
#else
...
EXPORT_SYMBOL_GPL(exynos_gpu_add_notifier);
			   ...
			   const struct cpumask *clip_gpus, u32 capacitance)
{
	...
	if (capacitance) {
		...
		ret = exynos_build_static_power_table(np, &gpufreq_cdev->var_table, &gpufreq_cdev->var_volt_size,
				&gpufreq_cdev->var_temp_size, "G3D");
		if (ret)
			goto err_release_idr;
	}
...
int exynos_gpu_cooling_init(void)
{
