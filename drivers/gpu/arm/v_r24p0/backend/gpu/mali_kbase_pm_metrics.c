...
int kbasep_pm_metrics_init(struct kbase_device *kbdev)
{
	...
	kbdev->pm.backend.metrics.values.busy_js[0] = 0;
	kbdev->pm.backend.metrics.values.busy_js[1] = 0;
	kbdev->pm.backend.metrics.values.busy_js[2] = 0;

	kbdev->pm.backend.metrics.values.idle_js[0] = 0;
	kbdev->pm.backend.metrics.values.idle_js[1] = 0;
	kbdev->pm.backend.metrics.values.idle_js[2] = 0;
...
static void kbase_pm_metrics_active_calc(struct kbase_device *kbdev)
{
	...
	kbdev->pm.backend.metrics.active_cl_ctx[0] = 0;
	kbdev->pm.backend.metrics.active_cl_ctx[1] = 0;
	kbdev->pm.backend.metrics.active_js_ctx[0] = false;
	kbdev->pm.backend.metrics.active_js_ctx[1] = false;
	kbdev->pm.backend.metrics.active_js_ctx[2] = false;
	kbdev->pm.backend.metrics.gpu_active = false;
	...
	for (js = 0; js < BASE_JM_MAX_NR_SLOTS; js++) {
		...
		if (katom && katom->gpu_rb_state ==
			...
			if (js == 0) kbdev->pm.backend.metrics.active_js_ctx[0] = true;
			if (js == 1) kbdev->pm.backend.metrics.active_js_ctx[1] = true;
			if (js == 2) kbdev->pm.backend.metrics.active_js_ctx[2] = true;
