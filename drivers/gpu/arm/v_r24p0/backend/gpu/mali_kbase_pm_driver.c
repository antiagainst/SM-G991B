...
void kbase_g3d_config_set_coherency_features(const struct kbase_device *kbdev) {
	...
	/* Set COHERENCY_FEATURES by G3D_CONFIG(0x18420400) */
	g3d_coherency_features = ioremap(0x18420400, 4);

	/* G3D_CONFIG[0]: support ACE-Lite / G3D_CONFIG[1]: support ACE */
	__raw_writel(0x3, g3d_coherency_features);

	g3d_coherency_features_val = (((unsigned long long)__raw_readl(g3d_coherency_features)));

	iounmap(g3d_coherency_features);
