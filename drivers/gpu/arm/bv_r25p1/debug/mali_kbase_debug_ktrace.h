...
/* MALI_SEC_INTEGRATION */
#define KBASE_KTRACE_ADD_EXYNOS(kbdev, code, ctx, katom, gpu_addr, info_val)	\
	do { \
		u64 __info_val = info_val; \
		CSTD_UNUSED(katom);\
		CSTD_UNUSED(gpu_addr);\
		/* capture values that could come from non-pure function calls */ \
		KBASE_KTRACE_RBUF_ADD(kbdev, code, ctx, __info_val); \
		KBASE_KTRACE_FTRACE_ADD(kbdev, code, ctx, __info_val); \
	} while (0)
