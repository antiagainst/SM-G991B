...
static int gpu_pm_notifier(struct notifier_block *nb, unsigned long event, void *cmd)
{
	...
	case PM_SUSPEND_PREPARE:
		if (platform) {
			...
			GPU_LOG(DVFS_DEBUG, LSI_GPU_OFF, platform->cur_clock, platform->gpu_dvfs_start_clock, "gpu_pm_notifier PM_SUSPEND_PREPARE - cur clock = %d, set middle clock = %d\n", platform->cur_clock, platform->gpu_dvfs_start_clock);
			gpu_set_target_clk_vol(platform->gpu_dvfs_start_clock, false, true);
...
static int pm_callback_dvfs_on(struct kbase_device *kbdev)
{
	...
	if (platform->dvfs_status && platform->wakeup_lock) {
		if (platform->restore_clock > platform->gpu_dvfs_start_clock) {
...
static void pm_callback_runtime_off(struct kbase_device *kbdev)
{
	...
	GPU_LOG(DVFS_DEBUG, LSI_GPU_OFF, platform->cur_clock, platform->gpu_dvfs_start_clock, "runtime off callback - cur clock = %d, set middle clock = %d\n", platform->cur_clock, platform->gpu_dvfs_start_clock);
	gpu_set_target_clk_vol(platform->gpu_dvfs_start_clock, false, true);
#endif /* CONFIG_MALI_DVFS */
