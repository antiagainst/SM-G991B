...
static int gpu_dvfs_update_config_data_from_dt(struct kbase_device *kbdev)
{
...
#if IS_ENABLED(CONFIG_CAL_IF) && !(IS_ENABLED(CONFIG_MALI_FG_DVFS))
...
#ifdef CONFIG_MALI_TSG
	gpu_update_config_data_int(np, "gpu_weight_table_idx_0", &platform->weight_table_idx_0);
	gpu_update_config_data_int(np, "gpu_weight_table_idx_1", &platform->weight_table_idx_1);
#endif
   ...
   gpu_update_config_data_int(np, "gpu_sustainable_clock", &platform->gpu_sustainable_clock);
#endif
	...
	platform->gpu_dss_freq_id = dbg_snapshot_get_freq_idx("G3D");
#endif
	...
	gpu_update_config_data_int(np, "gpu_heavy_compute_cpu0_min_clock", &platform->gpu_heavy_compute_cpu0_min_clock);
	gpu_update_config_data_int(np, "gpu_heavy_compute_vk_cpu0_min_clock", &platform->gpu_heavy_compute_vk_cpu0_min_clock);
#endif
	gpu_update_config_data_int(np, "sysreg_g3d", &platform->sysreg_g3d);
	return 0;
}

#if IS_ENABLED(CONFIG_MALI_FG_DVFS)
static int gpu_dvfs_update_asv_table(struct kbase_device *kbdev)
{
	...
	/* Allocate memory for freq volt table */
	freq_volt_table = kzalloc(sizeof(struct freq_volt) * dvfs_table_row_num, GFP_KERNEL);

	if (!freq_volt_table) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "freq volt table allocation failed\n");
		return -ENOMEM;
	}
	...
	for (i = 0; i < dvfs_table_row_num; i++) {
		int idx = i * dvfs_table_col_num;
		freq_volt_table[i].freq = of_data_int_array[idx];
	}

	if (cal_dfs_get_freq_volt_table(platform->g3d_cmu_cal_id, freq_volt_table,
			dvfs_table_row_num)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u,
			"failed to get fine-grain freq volt table from cal\n");
		return -1;
	}

	for (i = 0; i < dvfs_table_row_num; i++) {
		int cal_freq = freq_volt_table[i].freq;
		int cal_vol = freq_volt_table[i].volt;
		if (cal_freq <= platform->gpu_max_clock && cal_freq >= platform->gpu_min_clock) {
			table_idx = i * dvfs_table_col_num;
			dvfs_table[i].clock = cal_freq;
			dvfs_table[i].voltage = cal_vol;
			dvfs_table[i].min_threshold = of_data_int_array[table_idx+1];
			dvfs_table[i].max_threshold = of_data_int_array[table_idx+2];
			dvfs_table[i].down_staycount = of_data_int_array[table_idx+3];
			dvfs_table[i].mem_freq = of_data_int_array[table_idx+4];
			dvfs_table[i].cpu_little_min_freq = of_data_int_array[table_idx+5];
			...
			GPU_LOG(DVFS_INFO, DUMMY, 0u, 0u, "up [%d] down [%d] staycnt [%d] mif [%d] lit [%d] mid [%d] big [%d]\n",
					dvfs_table[i].max_threshold, dvfs_table[i].min_threshold, dvfs_table[i].down_staycount,
					dvfs_table[i].mem_freq, dvfs_table[i].cpu_little_min_freq, dvfs_table[i].cpu_middle_min_freq,
					dvfs_table[i].cpu_big_max_freq);
			} else {
				//Assuming cpu cluster number is 2
				dvfs_table[i].cpu_big_max_freq = (of_data_int_array[table_idx+6] ? of_data_int_array[table_idx+6]:CPU_MAX);
				GPU_LOG(DVFS_INFO, DUMMY, 0u, 0u, "up [%d] down [%d] staycnt [%d] mif [%d] lit [%d] big [%d]\n",
						dvfs_table[i].max_threshold, dvfs_table[i].min_threshold, dvfs_table[i].down_staycount,
						dvfs_table[i].mem_freq, dvfs_table[i].cpu_little_min_freq, dvfs_table[i].cpu_big_max_freq);
			}
		}
	}
#ifdef CONFIG_MALI_CL_PMQOS
	...
	for (i = 0; i < cl_pmqos_table_row_num; i++) {
		int cal_freq = freq_volt_table[i].freq;
		if (cal_freq <= platform->gpu_max_clock && cal_freq >= platform->gpu_min_clock) {
			table_idx = i * cl_pmqos_table_col_num;
			if (cal_freq == of_data_int_array[table_idx]) {
				platform->cl_pmqos_table[i].clock = cal_freq;
				platform->cl_pmqos_table[i].mif_min_freq = of_data_int_array[table_idx+1];
				/* platform->cl_pmqos_table[i].mif_max_freq = (of_data_int_array[table_idx+2] ? of_data_int_array[table_idx+2]:INT_MAX); */
				platform->cl_pmqos_table[i].cpu_little_min_freq = of_data_int_array[table_idx+2];
				/* platform->cl_pmqos_table[i].cpu_little_max_freq = (of_data_int_array[table_idx+4] ? of_data_int_array[table_idx+4]:INT_MAX); */

				if (platform->gpu_pmqos_cpu_cluster_num == 3) {
					platform->cl_pmqos_table[i].cpu_middle_min_freq = of_data_int_array[table_idx+3];
					/* platform->cl_pmqos_table[i].cpu_middle_max_freq = (of_data_int_array[table_idx+6] ? of_data_int_array[table_idx+6]:INT_MAX); */
					/* platform->cl_pmqos_table[i].cpu_big_min_freq = of_data_int_array[table_idx+7]; */
					platform->cl_pmqos_table[i].cpu_big_max_freq = (of_data_int_array[table_idx+4] ? of_data_int_array[table_idx+4]:CPU_MAX);
					GPU_LOG(DVFS_INFO, DUMMY, 0u, 0u, "CL PMQOS TABLE G3D %7dKhz mif-min [%d] lit-min [%d] mid-min [%d] big-max [%d]\n",
							...
							platform->cl_pmqos_table[i].cpu_big_max_freq);
				} else {
					/* platform->cl_pmqos_table[i].cpu_big_min_freq = of_data_int_array[table_idx+5]; */
					platform->cl_pmqos_table[i].cpu_big_max_freq = (of_data_int_array[table_idx+4] ? of_data_int_array[table_idx+4]:CPU_MAX);
...
#elif IS_ENABLED(CONFIG_MALI_DVFS)
static int gpu_dvfs_update_asv_table(struct kbase_device *kbdev)
{
	struct exynos_context *platform = kbdev->platform_context;
	gpu_dvfs_info *dvfs_table;
...
#if IS_ENABLED(CONFIG_MALI_USES_LLC)
	spin_lock_init(&platform->llc_spinlock);
#endif

#if IS_ENABLED(CONFIG_EXYNOS_BTS)
	spin_lock_init(&platform->bts_spinlock);
#endif
	return 0;
}
