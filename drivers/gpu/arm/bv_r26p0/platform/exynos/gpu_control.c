...
int gpu_get_cur_clock(struct exynos_context *platform)
{
	if (!platform)
		return -ENODEV;
#if IS_ENABLED(CONFIG_CAL_IF)
	return cal_dfs_cached_get_rate(platform->g3d_cmu_cal_id);
#else
	return 0;
#endif
}
...
static int gpu_set_dvfs_using_calapi(struct exynos_context *platform, int clk)
{
	...
	platform->cur_clock = cal_dfs_cached_get_rate(platform->g3d_cmu_cal_id);
...
{
#ifdef CONFIG_MALI_RT_PM
#if IS_ENABLED(CONFIG_MALI_IFPO)
#if IS_ENABLED(CONFIG_CAL_IF)
	int status;
#endif
