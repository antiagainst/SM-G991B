...
/* Temporary lock check value */
static atomic_t mali_temp_lock_checker = ATOMIC_INIT(0);
ktime_t mali_last_lock_time = 0;
		...
		struct shrink_control *sc)
{
	...
	atomic_inc(&mali_temp_lock_checker);
	mali_last_lock_time = ktime_get();
	mutex_lock(&kctx->jit_evict_lock);
	...
	mutex_unlock(&kctx->jit_evict_lock);
	atomic_dec(&mali_temp_lock_checker);
		...
		u64 *flags)
{
	...
	KBASE_DEBUG_ASSERT(flags);
	KBASE_DEBUG_ASSERT(phandle);

	if (!phandle) {
		dev_warn(kctx->kbdev->dev,
			"kbase_mem_import called with NULL phandle (%llx)",
			phandle);
		goto bad_flags;
	}
