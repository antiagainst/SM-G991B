...
static int kbase_pm_l2_update_state(struct kbase_device *kbdev)
{
	...
	if (kbdev->pm.backend.invoke_poweroff_wait_wq_when_l2_off &&
		...
		KBASE_KTRACE_ADD_EXYNOS(kbdev, KBASE_DEVICE_PM_WAIT_WQ_QUEUE_WORK, NULL, NULL, 0u, 0u);
...
void kbase_pm_clock_on(struct kbase_device *kbdev, bool is_resume)
{
	...
	/* MALI_SEC_INTEGRATION */
	GPU_LOG(DVFS_INFO, LSI_RESUME_CHECK, is_resume, kbdev->pm.backend.metrics.timer_active, "resume_check(%d)\n", is_resume);

	/* MALI_SEC_INTEGRATION */
#if IS_ENABLED(CONFIG_MALI_DELAYED_IFPO_ON_RESUME)
	if (is_resume)
		platform->ifpo_delay = 30;
#endif
    ...
    kbdev->gpu_power_on_time = ktime_get();
	...
	if (platform) {
		GPU_LOG(DVFS_INFO, LSI_RESUME_FREQ, kbdev->pm.backend.metrics.timer_active, platform->cur_clock, "after resume clock(%d):%d\n", kbdev->pm.backend.metrics.timer_active, platform->cur_clock);
		if (kbdev->pm.backend.metrics.timer_active == false) {
			dev_err(kbdev->dev, "resume state(%d) with dvfs timer off(resume clock:%d)!!!\n", is_resume, platform->cur_clock);
			if (kbdev->pm.backend.callback_power_dvfs_on) {
				kbdev->pm.backend.callback_power_dvfs_on(kbdev);
				GPU_LOG(DVFS_INFO, LSI_RESUME_FREQ, kbdev->pm.backend.metrics.timer_active, platform->cur_clock, "re-enable dvfs timer - resume clock(%d):%d\n",
						kbdev->pm.backend.metrics.timer_active, platform->cur_clock);
			}
		}
	}
...
void kbase_g3d_config_set_coherency_features(const struct kbase_device *kbdev) {
	if (kbdev->g3d_coherency_features) {
		unsigned long long g3d_coherency_features_val;

		/* G3D_CONFIG[0]: support ACE-Lite / G3D_CONFIG[1]: support ACE */
		__raw_writel(0x3, kbdev->g3d_coherency_features);

		g3d_coherency_features_val = (((unsigned long long)__raw_readl(kbdev->g3d_coherency_features)));

		GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u,
			"%s: available COHERENCY_FEATURES: %#x\n", __func__, g3d_coherency_features_val);
	}
}
#endif

#if IS_ENABLED(CONFIG_MALI_USES_LLC)
extern ssize_t set_aruser_hint(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
extern ssize_t set_awuser_hint(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);

void kbase_g3d_config_set_aruser(const struct kbase_device *kbdev, uint64_t val) {
	set_aruser_hint(NULL, NULL, "0xffffffff", 0);
}

void kbase_g3d_config_set_awuser(const struct kbase_device *kbdev, uint64_t val) {
	set_awuser_hint(NULL, NULL, "0x3ffffffff", 0);
}
#endif
...
int kbase_pm_init_hw(struct kbase_device *kbdev, unsigned int flags)
{
	...
	/* MALI_SEC_INTEGRATION */
#if IS_ENABLED(CONFIG_MALI_COHERENCY)
	kbase_g3d_config_set_coherency_features(kbdev);
#endif
	kbase_cache_set_coherency_mode(kbdev, kbdev->system_coherency);

#if IS_ENABLED(CONFIG_MALI_USES_LLC)
	kbase_g3d_config_set_awuser(kbdev, 0x3ffffffff);
	kbase_g3d_config_set_aruser(kbdev, 0xffffffff);
#endif
