		...
		struct kbase_fault *fault)
{
	...
	kbdev->unhandled_page_fault_time = ktime_get();

	printk("[debug] gpu power on time : %u\n", (u32)(ktime_to_ns(kbdev->gpu_power_on_time)));
	printk("[debug] unhandled page fault  time : %u\n", (u32)(ktime_to_ns(kbdev->unhandled_page_fault_time)));
	...
	dev_err(kbdev->dev,
		...
		kctx->pid,
		/*---DEBUG---*/
		cpufreq_get(0), /* little */
		cpufreq_get(4), /* middle */
		cpufreq_get(7), /* big */
		platform->cur_clock, /* gpu */
		exynos_devfreq_get_domain_freq(0), /* mif */
		exynos_devfreq_get_domain_freq(1) /* int */
		/*-----------*/
		);
