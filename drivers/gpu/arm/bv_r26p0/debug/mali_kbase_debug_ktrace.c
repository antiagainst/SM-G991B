...
#include "mali_kbase_debug_ktrace_defs.h"

bool check_trace_code(kbase_ktrace_code_t code)
{
	...
	case KBASE_KTRACE_CODE(LSI_PM_SUSPEND):
		return true;
	case KBASE_KTRACE_CODE(LSI_GPU_DVS_ON): /* gpu dvs on */
	case KBASE_KTRACE_CODE(LSI_GPU_DVS_OFF): /* gpu dvs off */
	case KBASE_KTRACE_CODE(LSI_SECURE_WORLD_ENTER): /* SECURE RENDERING START */
	case KBASE_KTRACE_CODE(LSI_SECURE_WORLD_EXIT): /* SECURE RENDERING END */
	case KBASE_KTRACE_CODE(LSI_SECURE_CACHE): /* SECURE RENDERING CACHE FLUSH */
	case KBASE_KTRACE_CODE(LSI_SECURE_CACHE_END): /* SECURE RENDERING CACHE FLUSH END */
	case KBASE_KTRACE_CODE(LSI_KBASE_PM_INIT_HW): /* SECURE RENDERING END */
	case KBASE_KTRACE_CODE(LSI_IFPM_POWER_ON): /* IFPM Power on */
	case KBASE_KTRACE_CODE(LSI_IFPM_POWER_OFF): /* IFPM Power off */
	case KBASE_KTRACE_CODE(LSI_GPU_SECURE): /* GPU Secure Rendering */
		return true;
	default:
		return false;
	}
	return true;
#endif /* CONFIG_MALI_EXYNOS_TRACE_ALL */
}
#endif /* CONFIG_MALI_EXYNOS_TRACE */
