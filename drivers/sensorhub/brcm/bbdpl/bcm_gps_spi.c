...
unsigned long bcm_clock_get_ms(void)
{
	...
	t = ktime_to_timespec(ktime_get_boottime());
	now = timespec_to_ns(&t);
	if (init_time == 0)
		init_time = now;
...
static ssize_t show_nstanby(struct device *dev,
	struct device_attribute *attr, char *buf){
	if (gps_pwr_en >= 0) {
		pr_err("[SSP] %s: gps_pwr_en(%d), is enabled %d\n", __func__,
				gps_pwr_en, gpio_get_value(gps_pwr_en));
		return sprintf(buf, "%d\n", gpio_get_value(gps_pwr_en));
	}

	return 0;
}

static ssize_t set_nstanby(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	gpio_set_value(gps_pwr_en, enable);
	pr_err("[SSP] %s: gps_pwr_en(%d), is enabled %d\n", __func__,
				gps_pwr_en, gpio_get_value(gps_pwr_en));
...
static DEVICE_ATTR(value, 0664, show_nstanby, set_nstanby);
...
static int bcm_spi_probe(struct spi_device *spi)
{
	...
	struct device *gps_dev, *gps_pwr_en_dev;	
	...
	*/
	...
	gps_pwr_en_dev = device_create(gps_dev->class, gps_dev, 0, NULL, "%s", "GPS_PWR_EN");
	device_create_file(gps_pwr_en_dev, &dev_attr_value);
	/*
...
int request_firmware_update(const char *path, const struct firmware **fw_entry) {
	/* request firmware */
	int ret =  0;

	ret = request_firmware(fw_entry, path, &g_bcm_gps->spi->dev);
	if (ret < 0) {
		pr_err("[SSP] SPU request_firmware failed(%d)", ret);
	} else {
		ret = spu_firmware_signature_verify("SENSORHUB", (*fw_entry)->data, (*fw_entry)->size);
		if (ret < 0) {
			pr_err("[SSP] SPU signature verify error(%d)", ret);
			*fw_entry = NULL;
		}
	}
	return ret;
...
void bcm4773_debug_info(void)
{
	...
	if (g_bcm_gps) {
		...
		irq_count = 0;//kstat_irqs_cpu(g_bcm_gps->spi->irq, 0);
