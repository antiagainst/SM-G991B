...
struct bbd_device {
	...
	struct wakeup_source *bbd_wake_lock;
	struct wakeup_source *patch_wake_lock;
	bool db;				/* debug flag */
...
static unsigned long clock_get_ms(void)
{
	...
	now = timespec_to_ns(&t) / NSEC_PER_MSEC;
	if (init_time == 0)
		init_time = now;
...
static struct timespec64 bbd_sensor_time;
ssize_t bbd_sensor_write(const char *buf, size_t size)
{
	...
	bbd_sensor_time = ktime_to_timespec64(ktime_get_boottime());
...
{
	struct timespec64 curr_ts = ktime_to_timespec64(ktime_get_boottime());
	struct timespec64 delta = timespec64_sub(curr_ts, bbd_sensor_time);

	return timespec64_to_ns(&delta)/NSEC_PER_USEC;
}
...
ssize_t bbd_patch_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (bbd.ssp_cb) {
		...
		int patch_size = get_patch(&bbd_patch);

		rd_size = patch_size < size ? patch_size : size;
...
ssize_t bbd_urgent_patch_read(struct file *user_filp, char __user *buf, size_t size, loff_t *ppos)
{
	...
	int ret = request_firmware_update(URGENT_FIRMWARE_PATH, &fw_entry);
	...
	if (ret < 0) {
		...
		if(copy_to_user(buf, (void *)(fw_entry->data + offset), rd_size)) {
...
int bbd_init(struct device *dev)
{
	...
	wake_lock_init(bbd.bbd_wake_lock, NULL, "bbd_wake_lock");
	wake_lock_init(bbd.patch_wake_lock, NULL, "patch_wake_lock");
