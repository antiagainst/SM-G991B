...
int ssp_sensorhub_initialize(struct ssp_data *ssp_data)
{
	...
	wake_lock_init(hub_data->sensorhub_wake_lock, NULL, "ssp_sensorhub_wake_lock");
	...
	init_completion(&hub_data->fm_read_done);
	init_completion(&hub_data->fm_write_done);
	hub_data->fm_tx_buffer_length = 0;
	memset(&hub_data->fm_tx_buffer, 0, sizeof(hub_data->fm_tx_buffer));
	memset(&hub_data->fm_rx_buffer, 0, sizeof(hub_data->fm_rx_buffer));
