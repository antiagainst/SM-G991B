				 ...
				 enum S2M_RTC_OP op)
{
	...
	ret = s2mps23_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, data);
	if (ret < 0)
...
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	mutex_lock(&info->lock);
	ret = s2mps23_bulk_write(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_check_rtc_time(struct s2m_rtc_info *info)
{
	...
	if (abs(rtc_time - sys_time.tv_sec) > 2) {
		...
		ret = s2mps23_bulk_write(info->i2c, S2MP_RTC_REG_SEC,
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mps23_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
	...
	ret = s2mps23_read_reg(info->pmic_i2c, reg, &val); /* i2c for PM */
	if (ret < 0) {
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (info->alarm_check) {
		...
		ret = s2mps23_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
				data);
		if (ret < 0) {
						...
						struct s2mps23_platform_data *pdata)
{
	...
	if (pdata->osc_bias_up >= 0) {
		ret = s2mps23_update_reg(info->i2c, S2MP_RTC_REG_CAPSEL,
	...
	if (pdata->osc_xin >= 0) {
		ret = s2mps23_update_reg(info->i2c, S2MP_RTC_REG_OSCCTRL,
...
static bool s2m_is_jigonb_low(struct s2m_rtc_info *info)
{
	...
	ret = s2mps23_read_reg(info->pmic_i2c, reg, &val);
	if (ret < 0) {
						...
						struct s2mps23_platform_data *pdata)
{
	...
	ret = s2mps23_write_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, val);
	if (ret < 0) {
					...
					struct s2mps23_platform_data *pdata)
{
	...
	ret = s2mps23_update_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, 0,
				...
				struct s2mps23_platform_data *pdata)
{
	...
	ret = s2mps23_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &update_val);
	if (ret < 0) {
	...
	ret = s2mps23_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, info->update_reg);
	if (ret < 0) {
	...
	ret = s2mps23_read_reg(info->i2c, S2MP_RTC_REG_CTRL, &ctrl_val);
	if (ret < 0) {
	...
	ret = s2mps23_read_reg(info->i2c, S2MP_RTC_REG_CAPSEL, &capsel_val);
	if (ret < 0) {
	...
	data = MODEL24_MASK;
	ret = s2mps23_write_reg(info->i2c, S2MP_RTC_REG_CTRL, data);
	if (ret < 0) {
	...
	capsel_val |= 0xf0;
	ret = s2mps23_write_reg(info->i2c, S2MP_RTC_REG_CAPSEL, capsel_val);
	if (ret < 0) {
...
static int s2m_rtc_probe(struct platform_device *pdev)
{
	...
	struct s2mps23_platform_data *pdata = dev_get_platdata(iodev->dev);
	...
	switch (info->iodev->device_type) {
	case S2MPS23X:
		info->irq = irq_base + S2MPS23_PMIC_IRQ_RTCA0_INT2;
		break;
	default:
		/* If this happens the core funtion has a problem */
		BUG();
	...
	info->rtc_dev = devm_rtc_device_register(&pdev->dev, "s2mps23-rtc",
...
static int s2mps23_rtc_suspend(struct device *dev)
{
	struct s2m_rtc_info *info = dev_get_drvdata(dev);
	struct rtc_time tm;
	int ret;
...
static int s2mps23_rtc_resume(struct device *dev)
{
	struct s2m_rtc_info *info = dev_get_drvdata(dev);
	struct rtc_time tm;
	int ret;
...
static const struct dev_pm_ops s2mps23_rtc_pm = {
	.suspend = s2mps23_rtc_suspend,
	.resume = s2mps23_rtc_resume,
};
