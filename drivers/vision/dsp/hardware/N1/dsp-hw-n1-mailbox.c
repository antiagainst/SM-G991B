...
static struct dsp_mailbox_pool *dsp_hw_n1_mailbox_alloc_pool(
...
static void dsp_hw_n1_mailbox_free_pool(struct dsp_mailbox *mbox,
		struct dsp_mailbox_pool *pool)
{
	struct dsp_mailbox_pool_manager *pmgr;
	unsigned long flags;
...
static void dsp_hw_n1_mailbox_dump_pool(struct dsp_mailbox *mbox,
		struct dsp_mailbox_pool *pool)
{
	size_t idx;
...
static int __dsp_hw_n1_mailbox_check_version(struct dsp_mailbox *mbox,
...
static int __dsp_hw_n1_mailbox_send_mail(struct dsp_mailbox *mbox,
...
static int dsp_hw_n1_mailbox_send_message(struct dsp_mailbox *mbox,
		unsigned int message_id)
{
	...
	ret = __dsp_hw_n1_mailbox_send_mail(mbox, &mail);
	if (ret)
		goto p_err_send;
...
static int dsp_hw_n1_mailbox_send_task(struct dsp_mailbox *mbox,
		struct dsp_task *task)
{
	...
	mail.message_size = (unsigned int)task->pool->used_size;
...
static int dsp_hw_n1_mailbox_receive_task(struct dsp_mailbox *mbox)
{
	int ret;
	...
	ret = __dsp_hw_n1_mailbox_check_version(mbox, mail.mailbox_version,
...
static int __dsp_hw_n1_mailbox_set_version(struct dsp_mailbox *mbox)
{
	int ret;
	unsigned int mailbox_version, message_version;

	dsp_enter();
	mailbox_version = dsp_ctrl_sm_readl(
			DSP_N1_SM_RESERVED(DSP_N1_SM_MAILBOX_VERSION));
	message_version = dsp_ctrl_sm_readl(
			DSP_N1_SM_RESERVED(DSP_N1_SM_MESSAGE_VERSION));

	ret = __dsp_hw_n1_mailbox_check_version(mbox, mailbox_version,
			message_version);
	if (ret)
		goto p_err;
...
static int dsp_hw_n1_mailbox_start(struct dsp_mailbox *mbox)
{
	int ret;
	struct dsp_priv_mem *pmem;

	dsp_enter();
	ret = __dsp_hw_n1_mailbox_set_version(mbox);
	if (ret)
		goto p_err;

	pmem = &mbox->sys->memory.priv_mem[DSP_N1_PRIV_MEM_MBOX_MEMORY];
	mbox->to_fw = mbox->sys->sfr + DSP_N1_SM_RESERVED(DSP_N1_SM_TO_CC_MBOX);
	...
	mbox->to_host = mbox->sys->sfr +
		DSP_N1_SM_RESERVED(DSP_N1_SM_TO_HOST_MBOX);
...
static int dsp_hw_n1_mailbox_stop(struct dsp_mailbox *mbox)
{
	dsp_enter();
	mbox->to_fw = NULL;
	mbox->to_host = NULL;
...
static int __dsp_hw_n1_mailbox_pool_manager_init(struct dsp_mailbox *mbox)
{
	int ret;
	struct dsp_priv_mem *pmem;
	struct dsp_mailbox_pool_manager *pmgr;
	...
	dsp_enter();
	pmem = &mbox->sys->memory.priv_mem[DSP_N1_PRIV_MEM_MBOX_POOL];
	pmgr = mbox->pool_manager;
...
static void __dsp_hw_n1_mailbox_pool_manager_deinit(
		struct dsp_mailbox_pool_manager *pmgr)
{
...
static int dsp_hw_n1_mailbox_open(struct dsp_mailbox *mbox)
{
	dsp_enter();
	mbox->mailbox_version = 0;
	mbox->message_version = 0;

	__dsp_hw_n1_mailbox_pool_manager_init(mbox);
	dsp_leave();
	return 0;
}

static int dsp_hw_n1_mailbox_close(struct dsp_mailbox *mbox)
{
	dsp_enter();
	__dsp_hw_n1_mailbox_pool_manager_deinit(mbox->pool_manager);
	dsp_leave();
	return 0;
}

static int dsp_hw_n1_mailbox_probe(struct dsp_system *sys)
{
	int ret;
...
static void dsp_hw_n1_mailbox_remove(struct dsp_mailbox *mbox)
{
	dsp_enter();
...
static const struct dsp_mailbox_ops n1_mailbox_ops = {
	.alloc_pool	= dsp_hw_n1_mailbox_alloc_pool,
	.free_pool	= dsp_hw_n1_mailbox_free_pool,
	.dump_pool	= dsp_hw_n1_mailbox_dump_pool,
	.send_task	= dsp_hw_n1_mailbox_send_task,
	.receive_task	= dsp_hw_n1_mailbox_receive_task,
	.send_message	= dsp_hw_n1_mailbox_send_message,

	.start		= dsp_hw_n1_mailbox_start,
	.stop		= dsp_hw_n1_mailbox_stop,

	.open		= dsp_hw_n1_mailbox_open,
	.close		= dsp_hw_n1_mailbox_close,
	.probe		= dsp_hw_n1_mailbox_probe,
	.remove		= dsp_hw_n1_mailbox_remove,
};

int dsp_hw_n1_mailbox_init(void)
{
	int ret;

	dsp_enter();
	ret = dsp_mailbox_register_ops(DSP_DEVICE_ID_N1, &n1_mailbox_ops);
	if (ret)
		goto p_err;
