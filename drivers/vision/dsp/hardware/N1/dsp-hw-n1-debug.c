...
static int dsp_hw_debug_devfreq_show(struct seq_file *file, void *unused)
{
	...
	for (count = 0; count < DSP_N1_DEVFREQ_COUNT; ++count) {
	...
	for (count = 0; count < DSP_N1_EXTRA_DEVFREQ_COUNT; ++count) {
...
static int dsp_hw_debug_mem_show(struct seq_file *file, void *unused)
{
	...
	for (idx = 0; idx < DSP_N1_PRIV_MEM_COUNT; ++idx)
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (id >= DSP_N1_PRIV_MEM_COUNT) {
		dsp_err("memory id(%u) of command is invalid(0 ~ %u)\n",
				id, DSP_N1_PRIV_MEM_COUNT - 1);
		goto out;
	}
...
static int dsp_hw_debug_userdefined_show(struct seq_file *file, void *unused)
{
	...
	seq_printf(file, "USERDEFINED : base_addr(%#x/%#x)/size(%u words)\n",
			(int)(sys->sfr_pa + DSP_N1_SM_USERDEFINED_BASE),
			DSP_N1_HW_BASE_ADDR + DSP_N1_SM_USERDEFINED_BASE,
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (num >= DSP_N1_SM_USERDEFINED_COUNT) {
		ret = -EINVAL;
		dsp_err("num(%u) of command is invalid(0 ~ %u)\n",
				num, DSP_N1_SM_USERDEFINED_COUNT - 1);
		goto p_err;
	}
	...
	if (dsp_device_power_active(dspdev)) {
		dsp_info("USERDEFINED(%u) is changed from %#x to %#x\n",
				num,
				dsp_ctrl_sm_readl(DSP_N1_SM_USERDEFINED(num)),
				val);
		dsp_ctrl_sm_writel(DSP_N1_SM_USERDEFINED(num), val);
	} else {
...
static int dsp_hw_debug_bus_show(struct seq_file *file, void *unused)
{
	...
	for (idx = 0; idx < DSP_N1_MO_SCENARIO_COUNT; ++idx)
		...
		dma_addr_t addr)
{
	...
	struct dsp_device *dspdev;
	struct dsp_memory *mem;
	struct dsp_hw_n1_debug *debug_sub;
	...
	debug_sub->npu_fw.size = PAGE_ALIGN(DSP_N1_NPU_FW_SIZE);
	...
	ret = dsp_binary_load(DSP_N1_NPU_FW_NAME, NULL, DSP_N1_FW_EXTENSION,
	...
	dspdev->system.ctrl.ops->extra_init(&dspdev->system.ctrl);
	dsp_dump_ctrl();
...
static void dsp_hw_n1_debug_log_flush(struct dsp_hw_debug *debug)
{
	int ret;
	struct dsp_hw_debug_log *log;
	char line[DSP_HW_DEBUG_LOG_LINE_SIZE];
...
static int dsp_hw_n1_debug_log_start(struct dsp_hw_debug *debug)
{
	struct dsp_hw_debug_log *log;
	struct dsp_system *sys;
	...
	log = debug->log;
	sys = &debug->dspdev->system;
	log_mem = &sys->memory.priv_mem[DSP_N1_PRIV_MEM_FW_LOG];

	log->queue = sys->sfr + DSP_N1_SM_RESERVED(DSP_N1_SM_LOG_QUEUE);
...
static int dsp_hw_n1_debug_log_stop(struct dsp_hw_debug *debug)
{
	dsp_enter();
...
static int dsp_hw_n1_debug_open(struct dsp_hw_debug *debug)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_n1_debug_close(struct dsp_hw_debug *debug)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_n1_debug_probe(struct dsp_device *dspdev)
{
	int ret;
	struct dsp_hw_debug *debug;
	struct dsp_hw_n1_debug *debug_sub;
...
static void dsp_hw_n1_debug_remove(struct dsp_hw_debug *debug)
{
	dsp_enter();
	kfree(debug->log);
...
static const struct dsp_hw_debug_ops n1_hw_debug_ops = {
	.log_flush	= dsp_hw_n1_debug_log_flush,
	.log_start	= dsp_hw_n1_debug_log_start,
	.log_stop	= dsp_hw_n1_debug_log_stop,

	.open		= dsp_hw_n1_debug_open,
	.close		= dsp_hw_n1_debug_close,
	.probe		= dsp_hw_n1_debug_probe,
	.remove		= dsp_hw_n1_debug_remove,
};

int dsp_hw_n1_debug_init(void)
{
	int ret;

	dsp_enter();
	ret = dsp_hw_debug_register_ops(DSP_DEVICE_ID_N1, &n1_hw_debug_ops);
	if (ret)
		goto p_err;
