...
#if !defined(CONFIG_EXYNOS_IOVMM) || !defined(CONFIG_ION_EXYNOS)
static bool __dsp_hw_n1_memory_ion_cached_dmabuf(struct dma_buf *dmabuf)
{
...
static int dsp_hw_n1_memory_map_buffer(struct dsp_memory *mem,
...
#else
	buf->cached = __dsp_hw_n1_memory_ion_cached_dmabuf(dbuf);
#endif
...
static int dsp_hw_n1_memory_unmap_buffer(struct dsp_memory *mem,
		struct dsp_buffer *buf)
{
	dsp_enter();
#if defined(CONFIG_EXYNOS_IOVMM) && defined(CONFIG_ION_EXYNOS)
...
static int dsp_hw_n1_memory_sync_for_device(struct dsp_memory *mem,
		struct dsp_buffer *buf)
{
	dsp_enter();
	if (!buf->cached)
...
#if defined(CONFIG_EXYNOS_DSP_HW_N1)
...
static int dsp_hw_n1_memory_sync_for_cpu(struct dsp_memory *mem,
		struct dsp_buffer *buf)
{
	dsp_enter();
	if (!buf->cached)
...
static int __dsp_hw_n1_memory_iovmm_map_sg(struct dsp_memory *mem,
...
static int __dsp_hw_n1_memory_iovmm_unmap(struct dsp_memory *mem,
		struct dsp_priv_mem *pmem)
{
	size_t size;
...
static int dsp_hw_n1_memory_alloc(struct dsp_memory *mem,
		struct dsp_priv_mem *pmem)
{
	int ret;
	void *kvaddr;
...
static void dsp_hw_n1_memory_free(struct dsp_memory *mem,
		struct dsp_priv_mem *pmem)
{
	dsp_enter();
	vfree(pmem->kvaddr);
...
#if !defined(CONFIG_EXYNOS_IOVMM) || !defined(CONFIG_ION_EXYNOS)
static unsigned int __dsp_hw_n1_get_heap_mask(void)
{
...
static int dsp_hw_n1_memory_ion_alloc(struct dsp_memory *mem,
		struct dsp_priv_mem *pmem)
{
	int ret;
#if defined(CONFIG_EXYNOS_IOVMM) && defined(CONFIG_ION_EXYNOS)
...
#else
	heap_mask = __dsp_hw_n1_get_heap_mask();
	if (!heap_mask) {
		ret = -EINVAL;
	...
	if (pmem->fixed_iova) {
		ret = __dsp_hw_n1_memory_iovmm_map_sg(mem, pmem);
		if (ret)
			goto p_err_map_dva;
	} else {
#if defined(CONFIG_EXYNOS_IOVMM) && defined(CONFIG_ION_EXYNOS)
...
static void dsp_hw_n1_memory_ion_free(struct dsp_memory *mem,
		struct dsp_priv_mem *pmem)
{
	dsp_enter();
	/* unused memory */
	...
	if (pmem->fixed_iova)
		__dsp_hw_n1_memory_iovmm_unmap(mem, pmem);
#if defined(CONFIG_EXYNOS_IOVMM) && defined(CONFIG_ION_EXYNOS)
	else
...
static int __dsp_hw_n1_memory_priv_alloc(struct dsp_memory *mem)
{
	int ret;
	struct dsp_priv_mem *pmem;
	int idx;
...
static void __dsp_hw_n1_memory_priv_free(struct dsp_memory *mem)
{
	struct dsp_priv_mem *pmem;
	int idx;
...
static int dsp_hw_n1_memory_open(struct dsp_memory *mem)
{
	int ret;

	dsp_enter();
	ret = __dsp_hw_n1_memory_priv_alloc(mem);
	if (ret)
		goto p_err;
...
static int dsp_hw_n1_memory_close(struct dsp_memory *mem)
{
	dsp_enter();
	__dsp_hw_n1_memory_priv_free(mem);
	dsp_leave();
	return 0;
}

static int dsp_hw_n1_memory_probe(struct dsp_system *sys)
{
	int ret;
	...
	mem = &sys->memory;
	mem->dev = sys->dev;
	mem->priv_mem_count = DSP_N1_PRIV_MEM_COUNT;
	...
	pmem_list = kcalloc(DSP_N1_PRIV_MEM_COUNT, sizeof(*pmem_list),
			GFP_KERNEL);
	if (!pmem_list) {
		ret = -ENOMEM;
	...
	pmem = &pmem_list[DSP_N1_PRIV_MEM_FW_STACK];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "fw_stack");
	pmem->size = PAGE_ALIGN(DSP_N1_FW_STACK_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_FW_STACK_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_FW_STACK_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_TO_DEVICE;
	pmem->kmap = false;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_FW_STACK_IOVA;
	pmem->backup = false;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_FW];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "fw");
	pmem->size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE - DSP_N1_FW_STACK_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_FW_SIZE);
	pmem->max_size =
		PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE - DSP_N1_FW_STACK_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_TO_DEVICE;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_FW_IOVA;
	pmem->backup = true;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_FW_LOG];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "fw_log");
	pmem->size = PAGE_ALIGN(DSP_N1_FW_LOG_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_FW_LOG_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_BIDIRECTIONAL;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_FW_LOG_IOVA;
	pmem->backup = false;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_IVP_PM];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "ivp_pm");
	pmem->size = PAGE_ALIGN(DSP_N1_IVP_PM_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_IVP_PM_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_TO_DEVICE;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_IVP_PM_IOVA;
	pmem->backup = false;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_IVP_DM];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "ivp_dm");
	pmem->size = PAGE_ALIGN(DSP_N1_IVP_DM_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_IVP_DM_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_TO_DEVICE;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_IVP_DM_IOVA;
	pmem->backup = false;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_IAC_PM];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "iac_pm");
	pmem->size = PAGE_ALIGN(DSP_N1_IAC_PM_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_IAC_PM_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_TO_DEVICE;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_IAC_PM_IOVA;
	pmem->backup = false;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_IAC_DM];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "iac_dm");
	pmem->size = PAGE_ALIGN(DSP_N1_IAC_DM_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_IAC_DM_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_TO_DEVICE;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_IAC_DM_IOVA;
	pmem->backup = false;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_MBOX_MEMORY];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "mbox_memory");
	pmem->size = PAGE_ALIGN(DSP_N1_MBOX_MEMORY_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_MBOX_MEMORY_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_BIDIRECTIONAL;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_MBOX_MEMORY_IOVA;
	pmem->backup = false;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_MBOX_POOL];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "mbox_pool");
	pmem->size = PAGE_ALIGN(DSP_N1_MBOX_POOL_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_MBOX_POOL_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_BIDIRECTIONAL;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_MBOX_POOL_IOVA;
	pmem->backup = false;

	pmem = &pmem_list[DSP_N1_PRIV_MEM_DL_OUT];
	snprintf(pmem->name, DSP_MEM_NAME_LEN, "dl_out");
	pmem->size = PAGE_ALIGN(DSP_N1_DL_OUT_SIZE);
	pmem->min_size = PAGE_ALIGN(DSP_N1_DL_OUT_SIZE);
	pmem->max_size = PAGE_ALIGN(DSP_N1_MEMORY_MAX_SIZE);
	pmem->flags = 0;
	pmem->dir = DMA_BIDIRECTIONAL;
	pmem->kmap = true;
	pmem->fixed_iova = true;
	pmem->iova = DSP_N1_DL_OUT_IOVA;
	pmem->backup = false;

	mem->priv_mem = pmem_list;
	mem->priv_mem_count = DSP_N1_PRIV_MEM_COUNT;
	mem->priv_ivp_pm_id = DSP_N1_PRIV_MEM_IVP_PM;
	mem->priv_dl_out_id = DSP_N1_PRIV_MEM_DL_OUT;
...
static void dsp_hw_n1_memory_remove(struct dsp_memory *mem)
{
	dsp_enter();
	kfree(mem->priv_mem);
	dsp_leave();
...
static const struct dsp_memory_ops n1_memory_ops = {
	.map_buffer		= dsp_hw_n1_memory_map_buffer,
	.unmap_buffer		= dsp_hw_n1_memory_unmap_buffer,
	.sync_for_device	= dsp_hw_n1_memory_sync_for_device,
	.sync_for_cpu		= dsp_hw_n1_memory_sync_for_cpu,

	.alloc			= dsp_hw_n1_memory_alloc,
	.free			= dsp_hw_n1_memory_free,
	.ion_alloc		= dsp_hw_n1_memory_ion_alloc,
	.ion_free		= dsp_hw_n1_memory_ion_free,
	.ion_alloc_secure	= dsp_hw_dummy_memory_ion_alloc_secure,
	.ion_free_secure	= dsp_hw_dummy_memory_ion_free_secure,

	.open			= dsp_hw_n1_memory_open,
	.close			= dsp_hw_n1_memory_close,
	.probe			= dsp_hw_n1_memory_probe,
	.remove			= dsp_hw_n1_memory_remove,
};
...
int dsp_hw_n1_memory_init(void)
{
	...
	dsp_enter();
	ret = dsp_memory_register_ops(DSP_DEVICE_ID_N1, &n1_memory_ops);
	if (ret)
		goto p_err;
