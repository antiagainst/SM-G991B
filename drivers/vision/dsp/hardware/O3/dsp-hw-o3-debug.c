...
static int dsp_hw_debug_devfreq_show(struct seq_file *file, void *unused)
{
	...
	for (count = 0; count < DSP_O3_DEVFREQ_COUNT; ++count) {
	...
	for (count = 0; count < DSP_O3_EXTRA_DEVFREQ_COUNT; ++count) {
...
static int dsp_hw_debug_mem_show(struct seq_file *file, void *unused)
{
	...
	for (idx = 0; idx < DSP_O3_PRIV_MEM_COUNT; ++idx)
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (id >= DSP_O3_PRIV_MEM_COUNT) {
		dsp_err("memory id(%u) of command is invalid(0 ~ %u)\n",
				id, DSP_O3_PRIV_MEM_COUNT - 1);
		goto out;
	}
...
static int dsp_hw_debug_wait_ctrl_show(struct seq_file *file, void *unused)
{
	...
	seq_printf(file, "[id:%u] wait mode %u (irq:%u/busy-waiting:%u)\n",
			...
			DSP_HW_O3_WAIT_MODE_INTERRUPT,
			DSP_HW_O3_WAIT_MODE_BUSY_WAITING);
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (dspdev->open_count) {
		...
		if (ctrl >= DSP_HW_O3_WAIT_MODE_NUM) {
			mutex_unlock(&dspdev->lock);
			ret = -EINVAL;
...
static int dsp_hw_debug_userdefined_show(struct seq_file *file, void *unused)
{
	...
	seq_printf(file, "USERDEFINED : base_addr(%#x)/size(%u words)\n",
			(int)(sys->sfr_pa + DSP_O3_SM_USERDEFINED_BASE),
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (num >= DSP_O3_SM_USERDEFINED_COUNT) {
		ret = -EINVAL;
		dsp_err("num(%u) of command is invalid(0 ~ %u)\n",
				num, DSP_O3_SM_USERDEFINED_COUNT - 1);
		goto p_err;
	}
	...
	if (dsp_device_power_active(dspdev)) {
		dsp_info("USERDEFINED(%u) is changed from %#x to %#x\n",
				num,
				dsp_ctrl_sm_readl(DSP_O3_SM_USERDEFINED(num)),
				val);
		dsp_ctrl_sm_writel(DSP_O3_SM_USERDEFINED(num), val);
	} else {
...
static int dsp_hw_debug_bus_show(struct seq_file *file, void *unused)
{
	...
	for (idx = 0; idx < DSP_O3_MO_SCENARIO_COUNT; ++idx)
...
static int dsp_hw_debug_npu_test_show(struct seq_file *file, void *unused)
{
	...
	seq_puts(file, "Command to control CA32 Core1 for NPU\n");
	seq_puts(file, " on) echo 1 > /d/dsp/hardware/npu_test\n");
...
#define NPU_CPU_CONFIG		(0x10863580)
...
static void __dsp_hw_debug_npu_enable(struct dsp_hw_debug *debug)
{
	...
	struct dsp_device *dspdev;
	struct dsp_memory *mem;
	struct dsp_hw_o3_debug *debug_sub;

	dsp_enter();
	dspdev = debug->dspdev;
	mem = &dspdev->system.memory;
	debug_sub = debug->sub_data;
	...
	snprintf(debug_sub->npu_fw.name, DSP_MEM_NAME_LEN, "npu_fw");
	debug_sub->npu_fw.size = PAGE_ALIGN(SZ_4K);
	debug_sub->npu_fw.flags = 0;
	debug_sub->npu_fw.dir = DMA_TO_DEVICE;
	debug_sub->npu_fw.kmap = true;
	debug_sub->npu_fw.fixed_iova = true;
	debug_sub->npu_fw.iova = 0x50000000;

	ret = mem->ops->ion_alloc(mem, &debug_sub->npu_fw);
	if (ret)
		return;

	ret = dsp_binary_load("npu", NULL, DSP_O3_FW_EXTENSION,
			debug_sub->npu_fw.kvaddr, debug_sub->npu_fw.size,
			&debug_sub->npu_fw.used_size);
	if (ret)
		goto p_err_free;
	...
	ret = dsp_device_start(dspdev, 0, NULL);
	if (ret)
		goto p_err_start;

	dsp_ctrl_remap_writel(NPU_CPU_CONFIG, 0x1);

	mutex_lock(&dspdev->lock);
	dspdev->system.boot_init |= BIT(DSP_SYSTEM_NPU_INIT);
	mutex_unlock(&dspdev->lock);
...
p_err_open:
p_err_free:
	mem->ops->ion_free(mem, &debug_sub->npu_fw);
}
...
static void __dsp_hw_debug_npu_disable(struct dsp_hw_debug *debug)
{
	...
	dsp_ctrl_remap_writel(NPU_CPU_CONFIG, 0x0);
	dsp_device_stop(debug->dspdev, 1);
	dsp_device_close(debug->dspdev);
	mem->ops->ion_free(mem, &debug_sub->npu_fw);
	dsp_leave();
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	ret = sscanf(command, "%d\n", &enable);
	if (ret != 1) {
		ret = -EINVAL;
		dsp_err("command[%s] to test npu core is invalid\n", command);
		goto p_err;
	}

	dsp_info("test CA32 1 core : %d\n", enable);
	if (enable)
		__dsp_hw_debug_npu_enable(debug);
	else
...
static void dsp_hw_o3_debug_log_flush(struct dsp_hw_debug *debug)
{
	int ret;
	struct dsp_hw_debug_log *log;
	char line[DSP_HW_DEBUG_LOG_LINE_SIZE];
...
static int dsp_hw_o3_debug_log_start(struct dsp_hw_debug *debug)
{
	struct dsp_hw_debug_log *log;
	struct dsp_system *sys;
	struct dsp_priv_mem *pmem;
	...
	pmem = &sys->memory.priv_mem[DSP_O3_PRIV_MEM_DHCP];
	log->queue = pmem->kvaddr + DSP_O3_DHCP_IDX(DSP_O3_DHCP_LOG_QUEUE);

	pmem = &sys->memory.priv_mem[DSP_O3_PRIV_MEM_FW_LOG];
...
static int dsp_hw_o3_debug_log_stop(struct dsp_hw_debug *debug)
{
	dsp_enter();
...
static int dsp_hw_o3_debug_open(struct dsp_hw_debug *debug)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_o3_debug_close(struct dsp_hw_debug *debug)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_o3_debug_probe(struct dsp_device *dspdev)
{
	int ret;
	struct dsp_hw_debug *debug;
	struct dsp_hw_o3_debug *debug_sub;
	...
	debug_sub->npu_test = debugfs_create_file("npu_test", 0640, debug->root,
			debug, &dsp_hw_debug_npu_test_fops);
	if (!debug_sub->npu_test)
...
static void dsp_hw_o3_debug_remove(struct dsp_hw_debug *debug)
{
	dsp_enter();
	kfree(debug->log);
...
static const struct dsp_hw_debug_ops o3_hw_debug_ops = {
	.log_flush	= dsp_hw_o3_debug_log_flush,
	.log_start	= dsp_hw_o3_debug_log_start,
	.log_stop	= dsp_hw_o3_debug_log_stop,

	.open		= dsp_hw_o3_debug_open,
	.close		= dsp_hw_o3_debug_close,
	.probe		= dsp_hw_o3_debug_probe,
	.remove		= dsp_hw_o3_debug_remove,
};

int dsp_hw_o3_debug_init(void)
{
	int ret;

	dsp_enter();
	ret = dsp_hw_debug_register_ops(DSP_DEVICE_ID_O3, &o3_hw_debug_ops);
	if (ret)
		goto p_err;
