...
static int dsp_hw_o3_system_request_control(struct dsp_system *sys,
		unsigned int id, union dsp_control *cmd)
{
	...
	case DSP_CONTROL_DISABLE_BOOST:
		mutex_lock(&sys->boost_lock);
		if (sys->boost) {
			sys->bus.ops->mo_put_by_id(&sys->bus, DSP_O3_MO_MAX);
			sys->pm.ops->boost_disable(&sys->pm);
			sys->boost = false;
		}
		mutex_unlock(&sys->boost_lock);
...
static int __dsp_hw_o3_system_wait_task(struct dsp_system *sys,
		struct dsp_task *task)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
	...
	if (sys_sub->wait_mode == DSP_HW_O3_WAIT_MODE_INTERRUPT) {
		timeout = wait_event_timeout(sys->task_manager.done_wq,
				task->state == DSP_TASK_STATE_COMPLETE,
				msecs_to_jiffies(wait_time));
	} else if (sys_sub->wait_mode == DSP_HW_O3_WAIT_MODE_BUSY_WAITING) {
	...
	if (!timeout) {
		...
		if (sys->debug_mode & BIT(DSP_DEBUG_MODE_TASK_TIMEOUT_PANIC)) {
			dsp_dump_set_value(DSP_O3_DUMP_DEFAULT_VALUE |
					1 << DSP_O3_DUMP_VALUE_SDMA);
			dsp_dump_ctrl();
...
static void __dsp_hw_o3_system_flush(struct dsp_system *sys)
{
...
static void __dsp_hw_o3_system_recovery(struct dsp_system *sys)
{
	int ret;
	struct dsp_task_manager *tmgr;
	unsigned long flags;
...
static int dsp_hw_o3_system_execute_task(struct dsp_system *sys,
		struct dsp_task *task)
{
	int ret;
	...
	if (task->wait) {
		ret = __dsp_hw_o3_system_wait_task(sys, task);
		if (ret) {
			if (task->recovery) {
				__dsp_hw_o3_system_flush(sys);
				__dsp_hw_o3_system_recovery(sys);
			}
			goto p_err;
		}
...
static void dsp_hw_o3_system_iovmm_fault_dump(struct dsp_system *sys)
{
	dsp_enter();
	dsp_dump_set_value(DSP_O3_DUMP_DEFAULT_VALUE |
			1 << DSP_O3_DUMP_VALUE_SDMA);
...
static int __dsp_hw_o3_system_master_copy(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
	...
	dsp_enter();
	sys_sub = sys->sub_data;
	rmem = &sys->memory.reserved_mem[DSP_O3_RESERVED_MEM_FW_MASTER];

	if (!sys_sub->boot_bin_size) {
		dsp_warn("master bin is not loaded yet\n");
		ret = dsp_binary_alloc_load(DSP_O3_MASTER_FW_NAME, NULL,
				DSP_O3_FW_EXTENSION, &sys_sub->boot_bin,
...
static void __dsp_hw_o3_system_init(struct dsp_system *sys)
{
	struct dsp_priv_mem *pmem;
	struct dsp_hw_o3_system *sys_sub;
	unsigned int chip_id;
	unsigned int product_id;
	unsigned int enable_mbox;

	dsp_enter();
	pmem = sys->memory.priv_mem;
	sys_sub = sys->sub_data;

	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_TO_CC_INT_STATUS), 0);
	dsp_ctrl_dhcp_writel(
			DSP_O3_DHCP_IDX(DSP_O3_DHCP_TO_HOST_INT_STATUS), 0);

	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_DEBUG_LAYER_START),
			sys->layer_start);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_DEBUG_LAYER_END),
			sys->layer_end);

	memcpy(pmem[DSP_O3_PRIV_MEM_FW].kvaddr,
			pmem[DSP_O3_PRIV_MEM_FW].bac_kvaddr,
			pmem[DSP_O3_PRIV_MEM_FW].used_size);

	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_FW_RESERVED_SIZE),
			pmem[DSP_O3_PRIV_MEM_FW].size);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_IVP_PM_IOVA),
			pmem[DSP_O3_PRIV_MEM_IVP_PM].iova);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_IVP_PM_SIZE),
			pmem[DSP_O3_PRIV_MEM_IVP_PM].used_size);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_IVP_DM_IOVA),
			pmem[DSP_O3_PRIV_MEM_IVP_DM].iova);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_IVP_DM_SIZE),
			pmem[DSP_O3_PRIV_MEM_IVP_DM].used_size);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_IAC_PM_IOVA),
			pmem[DSP_O3_PRIV_MEM_IAC_PM].iova);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_IAC_PM_SIZE),
			pmem[DSP_O3_PRIV_MEM_IAC_PM].used_size);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_IAC_DM_IOVA),
			pmem[DSP_O3_PRIV_MEM_IAC_DM].iova);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_IAC_DM_SIZE),
			pmem[DSP_O3_PRIV_MEM_IAC_DM].used_size);

	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_MAILBOX_VERSION), 0);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_MESSAGE_VERSION), 0);

	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_FW_LOG_MEMORY_IOVA),
			pmem[DSP_O3_PRIV_MEM_FW_LOG].iova);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_FW_LOG_MEMORY_SIZE),
			pmem[DSP_O3_PRIV_MEM_FW_LOG].size);
	dsp_ctrl_dhcp_writel(
			DSP_O3_DHCP_IDX(DSP_O3_DHCP_TO_CC_MBOX_MEMORY_IOVA),
			pmem[DSP_O3_PRIV_MEM_MBOX_MEMORY].iova);
	dsp_ctrl_dhcp_writel(
			DSP_O3_DHCP_IDX(DSP_O3_DHCP_TO_CC_MBOX_MEMORY_SIZE),
			pmem[DSP_O3_PRIV_MEM_MBOX_MEMORY].size);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_TO_CC_MBOX_POOL_IOVA),
			pmem[DSP_O3_PRIV_MEM_MBOX_POOL].iova);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_TO_CC_MBOX_POOL_SIZE),
			pmem[DSP_O3_PRIV_MEM_MBOX_POOL].size);

	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_KERNEL_MODE),
			DSP_DYNAMIC_KERNEL);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_DL_OUT_IOVA),
			pmem[DSP_O3_PRIV_MEM_DL_OUT].iova);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_DL_OUT_SIZE),
			pmem[DSP_O3_PRIV_MEM_DL_OUT].size);
	...
	chip_id = 0xdeadbeef;
#endif
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_CHIPID_REV), chip_id);
	dsp_info("CHIPID : %#x\n", chip_id);
...
#endif
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_PRODUCT_ID),
			product_id);
	dsp_info("PRODUCT_ID : %#x\n", product_id);

	enable_mbox = dsp_ctrl_readl(DSP_O3_MBOX_EXT_NS_INT_0_MR);
	...
	dsp_ctrl_writel(DSP_O3_MBOX_EXT_NS_INT_0_MR, enable_mbox);

	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_INTERRUPT_MODE),
			sys_sub->wait_mode);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_DRIVER_VERSION),
			sys->dspdev->version);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_FIRMWARE_VERSION),
			0xdeadbeef);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_RESERVED0),
			0xdeadbeef);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_RESERVED1),
			0xdeadbeef);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_RESERVED2),
			0xdeadbeef);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_RESERVED3),
			0xdeadbeef);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_RESERVED4),
			0xdeadbeef);
	dsp_ctrl_dhcp_writel(DSP_O3_DHCP_IDX(DSP_O3_DHCP_RESERVED5),
			0xdeadbeef);

	dsp_leave();
}

static int __dsp_hw_o3_system_wait_boot(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
	unsigned int wait_time;
...
static int __dsp_hw_o3_system_check_kernel_mode(struct dsp_system *sys)
{
	int kernel_mode;

	dsp_enter();
	kernel_mode = dsp_ctrl_dhcp_readl(
			DSP_O3_DHCP_IDX(DSP_O3_DHCP_KERNEL_MODE));
...
static int dsp_hw_o3_system_boot(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
	...
	ret = __dsp_hw_o3_system_master_copy(sys);
	if (ret)
		goto p_err;

	__dsp_hw_o3_system_init(sys);
	sys->debug.ops->log_start(&sys->debug);

	sys->pm.ops->update_devfreq_boot(&sys->pm);
	sys->ctrl.ops->start(&sys->ctrl);
	ret = __dsp_hw_o3_system_wait_boot(sys);
	...
	ret = __dsp_hw_o3_system_check_kernel_mode(sys);
	if (ret)
		goto p_err_reset;
...
static int __dsp_hw_o3_system_wait_reset(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
	unsigned int wait_time;
...
static int dsp_hw_o3_system_reset(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
	...
	ret = __dsp_hw_o3_system_wait_reset(sys);
	if (ret)
		sys->ctrl.ops->force_reset(&sys->ctrl);
	else
...
static int dsp_hw_o3_system_power_active(struct dsp_system *sys)
{
	dsp_check();
	return sys->pm.ops->devfreq_active(&sys->pm);
}

static int dsp_hw_o3_system_set_boot_qos(struct dsp_system *sys, int val)
{
	int ret;
...
static int dsp_hw_o3_system_runtime_resume(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
...
static int dsp_hw_o3_system_runtime_suspend(struct dsp_system *sys)
{
	struct dsp_hw_o3_system *sys_sub;
...
static int dsp_hw_o3_system_resume(struct dsp_system *sys)
{
	dsp_enter();
	__dsp_hw_o3_system_recovery(sys);
	dsp_leave();
...
static int dsp_hw_o3_system_suspend(struct dsp_system *sys)
{
	dsp_enter();
	__dsp_hw_o3_system_flush(sys);
	dsp_leave();
...
static int dsp_hw_o3_system_npu_start(struct dsp_system *sys, bool boot,
		dma_addr_t fw_iova)
{
	dsp_enter();
	dsp_leave();
...
static int __dsp_hw_o3_system_binary_copy(struct dsp_system *sys, void *list)
{
	int ret;
	...
	dsp_info("binary[%s/%u] is copied\n", DSP_O3_FW_NAME, bin->size);

	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_IVP_PM];
	bin = &bin_list->bins[BIN_TYPE_DSP_IVP_PM_BIN];
	if (bin->size > pmem->size) {
		ret = -EINVAL;
		dsp_err("binary(%s) size is over(%u/%zu)\n",
				DSP_O3_IVP_PM_NAME, bin->size, pmem->size);
		goto p_err;
	}
	...
	pmem->used_size = bin->size;
	dsp_info("binary[%s/%u] is copied\n", DSP_O3_IVP_PM_NAME, bin->size);

	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_IVP_DM];
	bin = &bin_list->bins[BIN_TYPE_DSP_IVP_DM_BIN];
	if (bin->size > pmem->size) {
		ret = -EINVAL;
		dsp_err("binary(%s) size is over(%u/%zu)\n",
				DSP_O3_IVP_DM_NAME, bin->size, pmem->size);
		goto p_err;
	}
	...
	pmem->used_size = bin->size;
	dsp_info("binary[%s/%u] is copied\n", DSP_O3_IVP_DM_NAME, bin->size);

	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_IAC_PM];
	bin = &bin_list->bins[BIN_TYPE_DSP_IAC_PM_BIN];
	if (bin->size > pmem->size) {
		ret = -EINVAL;
		dsp_err("binary(%s) size is over(%u/%zu)\n",
				DSP_O3_IAC_PM_NAME, bin->size,
				pmem->size);
		goto p_err;
	}
	...
	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_IAC_DM];
	bin = &bin_list->bins[BIN_TYPE_DSP_IAC_DM_BIN];
	if (bin->size > pmem->size) {
		ret = -EINVAL;
		dsp_err("binary(%s) size is over(%u/%zu)\n",
				DSP_O3_IAC_DM_NAME, bin->size, pmem->size);
		goto p_err;
	}
...
static int __dsp_hw_o3_system_binary_load(struct dsp_system *sys)
{
	int ret;
	struct dsp_memory *mem;
	struct dsp_priv_mem *pmem;
	...
	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_FW];
	ret = dsp_binary_load(DSP_O3_FW_NAME, sys->fw_postfix,
			DSP_O3_FW_EXTENSION, pmem->bac_kvaddr, pmem->size,
			&pmem->used_size);
	if (ret)
		goto p_err_load;

	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_IVP_PM];
	ret = dsp_binary_load(DSP_O3_IVP_PM_NAME, sys->fw_postfix,
			DSP_O3_FW_EXTENSION, pmem->kvaddr, pmem->size,
			&pmem->used_size);
	if (ret)
		goto p_err_load;

	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_IVP_DM];
	ret = dsp_binary_load(DSP_O3_IVP_DM_NAME, sys->fw_postfix,
			DSP_O3_FW_EXTENSION, pmem->kvaddr, pmem->size,
			&pmem->used_size);
	if (ret)
		goto p_err_load;
	...
	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_IAC_PM];
	ret = dsp_binary_load(DSP_O3_IAC_PM_NAME, sys->fw_postfix,
			DSP_O3_FW_EXTENSION, pmem->kvaddr, pmem->size,
			&pmem->used_size);
	if (ret)
		goto p_err_load;
	...
	pmem = &mem->priv_mem[DSP_O3_PRIV_MEM_IAC_DM];
	ret = dsp_binary_load(DSP_O3_IAC_DM_NAME, sys->fw_postfix,
			DSP_O3_FW_EXTENSION, pmem->kvaddr, pmem->size,
			&pmem->used_size);
	if (ret)
		goto p_err_load;
...
static int dsp_hw_o3_system_start(struct dsp_system *sys, void *bin_list)
{
	int ret;
	struct dsp_task_manager *tmgr;
	unsigned long flags;
	...
	if (bin_list)
		ret = __dsp_hw_o3_system_binary_copy(sys, bin_list);
	else
		ret = __dsp_hw_o3_system_binary_load(sys);
	if (ret)
		goto p_err_load;
...
static int dsp_hw_o3_system_stop(struct dsp_system *sys)
{
	dsp_enter();
	sys->interface.ops->stop(&sys->interface);
	dsp_leave();
...
static int dsp_hw_o3_system_open(struct dsp_system *sys)
{
	int ret;
...
static int dsp_hw_o3_system_close(struct dsp_system *sys)
{
	dsp_enter();
...
static void __dsp_hw_o3_system_master_load_async(const struct firmware *fw,
		void *context)
{
	...
	snprintf(full_name, DSP_BINARY_NAME_SIZE, "%s.%s",
			DSP_O3_MASTER_FW_NAME, DSP_O3_FW_EXTENSION);
...
#if IS_ENABLED(CONFIG_EXYNOS_IMGLOADER)
static int dsp_hw_o3_imgloader_mem_setup(struct imgloader_desc *desc,
...
static int dsp_hw_o3_imgloader_verify_fw(struct imgloader_desc *desc,
		phys_addr_t fw_phys_base, size_t fw_bin_size,
		size_t fw_mem_size)
{
	dsp_enter();
...
static int dsp_hw_o3_imgloader_blk_pwron(struct imgloader_desc *desc)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_o3_imgloader_init_image(struct imgloader_desc *desc)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_o3_imgloader_deinit_image(struct imgloader_desc *desc)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_o3_imgloader_shutdown(struct imgloader_desc *desc)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static struct imgloader_ops hw_o3_imgloader_ops = {
	.mem_setup	= dsp_hw_o3_imgloader_mem_setup,
	.verify_fw	= dsp_hw_o3_imgloader_verify_fw,
	.blk_pwron	= dsp_hw_o3_imgloader_blk_pwron,
	.init_image	= dsp_hw_o3_imgloader_init_image,
	.deinit_image	= dsp_hw_o3_imgloader_deinit_image,
	.shutdown	= dsp_hw_o3_imgloader_shutdown,
};

static int __dsp_hw_o3_system_imgloader_init(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
...
static void __dsp_hw_o3_system_imgloader_deinit(struct dsp_system *sys)
{
	struct dsp_hw_o3_system *sys_sub;
...
#if IS_ENABLED(CONFIG_EXYNOS_SYSTEM_EVENT)
static int dsp_hw_o3_system_sysevent_shutdown(const struct sysevent_desc *desc,
		bool force_stop)
{
	dsp_enter();
	dsp_leave();
...
static int dsp_hw_o3_system_sysevent_powerup(const struct sysevent_desc *desc)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static void dsp_hw_o3_system_sysevent_crash_shutdown(
...
static int dsp_hw_o3_system_sysevent_ramdump(int enable,
		const struct sysevent_desc *desc)
{
	dsp_enter();
	dsp_leave();
...
static int dsp_hw_o3_system_sysevent_notifier_cb(struct notifier_block *this,
...
static struct notifier_block sysevent_nb = {
	.notifier_call = dsp_hw_o3_system_sysevent_notifier_cb,
};

static int __dsp_hw_o3_system_sysevent_init(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
	...
	desc->dev = sys->dev;
	desc->owner = THIS_MODULE;
	desc->shutdown = dsp_hw_o3_system_sysevent_shutdown;
	desc->powerup = dsp_hw_o3_system_sysevent_powerup;
	desc->crash_shutdown = dsp_hw_o3_system_sysevent_crash_shutdown;
	desc->ramdump = dsp_hw_o3_system_sysevent_ramdump;
...
static void __dsp_hw_o3_system_sysevent_deinit(struct dsp_system *sys)
{
	struct dsp_hw_o3_system *sys_sub;
...
#if IS_ENABLED(CONFIG_EXYNOS_MEMORY_LOGGER)
static int dsp_hw_o3_system_log_status_notify(struct memlog_obj *obj,
		unsigned int flags)
{
	dsp_enter();
	dsp_leave();
...
static int dsp_hw_o3_system_log_level_notify(struct memlog_obj *obj,
		unsigned int flags)
{
	dsp_enter();
	dsp_leave();
...
static int dsp_hw_o3_system_log_enable_notify(struct memlog_obj *obj,
		unsigned int flags)
{
	dsp_enter();
	dsp_leave();
...
static int dsp_hw_o3_system_file_ops_completed(struct memlog_obj *obj,
		unsigned int flags)
{
	dsp_enter();
	dsp_leave();
...
static int __dsp_hw_o3_system_memlogger_init(struct dsp_system *sys)
{
	int ret;
	struct dsp_hw_o3_system *sys_sub;
	struct platform_device *pdev;
	...
	desc->ops.log_status_notify = dsp_hw_o3_system_log_status_notify;
	desc->ops.log_level_notify = dsp_hw_o3_system_log_level_notify;
	desc->ops.log_enable_notify = dsp_hw_o3_system_log_enable_notify;
	desc->ops.file_ops_completed = dsp_hw_o3_system_file_ops_completed;
...
static void __dsp_hw_o3_system_memlogger_deinit(struct dsp_system *sys)
{
	struct dsp_hw_o3_system *sys_sub;
...
static int dsp_hw_o3_system_probe(struct dsp_device *dspdev)
{
	int ret;
	struct dsp_system *sys;
	struct platform_device *pdev;
	...
	sys_sub->wait_mode = DSP_HW_O3_WAIT_MODE_INTERRUPT;
	sys->boost = false;
	...
	ret = dsp_binary_load_async(DSP_O3_MASTER_FW_NAME, NULL,
			DSP_O3_FW_EXTENSION, sys,
			__dsp_hw_o3_system_master_load_async);
	if (ret < 0)
		goto p_err_bin_load;

	ret = __dsp_hw_o3_system_imgloader_init(sys);
	if (ret)
		goto p_err_imgloader;

	ret = __dsp_hw_o3_system_sysevent_init(sys);
	if (ret)
		goto p_err_sysevent;

	__dsp_hw_o3_system_memlogger_init(sys);

	dsp_leave();
	return 0;
p_err_sysevent:
	__dsp_hw_o3_system_imgloader_deinit(sys);
p_err_imgloader:
p_err_bin_load:
...
static void dsp_hw_o3_system_remove(struct dsp_system *sys)
{
	struct dsp_hw_o3_system *sys_sub;
	...
	__dsp_hw_o3_system_memlogger_deinit(sys);
	__dsp_hw_o3_system_sysevent_deinit(sys);
	__dsp_hw_o3_system_imgloader_deinit(sys);
...
static const struct dsp_system_ops o3_system_ops = {
	.request_control	= dsp_hw_o3_system_request_control,
	.execute_task		= dsp_hw_o3_system_execute_task,
	.iovmm_fault_dump	= dsp_hw_o3_system_iovmm_fault_dump,
	.boot			= dsp_hw_o3_system_boot,
	.reset			= dsp_hw_o3_system_reset,
	.power_active		= dsp_hw_o3_system_power_active,
	.set_boot_qos		= dsp_hw_o3_system_set_boot_qos,
	.runtime_resume		= dsp_hw_o3_system_runtime_resume,
	.runtime_suspend	= dsp_hw_o3_system_runtime_suspend,
	.resume			= dsp_hw_o3_system_resume,
	.suspend		= dsp_hw_o3_system_suspend,

	.npu_start		= dsp_hw_o3_system_npu_start,
	.start			= dsp_hw_o3_system_start,
	.stop			= dsp_hw_o3_system_stop,

	.open			= dsp_hw_o3_system_open,
	.close			= dsp_hw_o3_system_close,
	.probe			= dsp_hw_o3_system_probe,
	.remove			= dsp_hw_o3_system_remove,
};

int dsp_hw_o3_system_init(void)
{
	int ret;

	dsp_enter();
	ret = dsp_system_register_ops(DSP_DEVICE_ID_O3, &o3_system_ops);
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_pm_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_clk_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_bus_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_llc_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_memory_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_interface_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_ctrl_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_mailbox_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_governor_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_debug_init();
	if (ret)
		goto p_err;

	ret = dsp_hw_o3_dump_init();
	if (ret)
		goto p_err;

	dsp_info("hw_o3(%u) was initilized\n", DSP_DEVICE_ID_O3);
	dsp_leave();
	return 0;
p_err:
	return ret;
