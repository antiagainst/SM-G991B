...
static struct dsp_reg_format n3_sfr_reg[] = {
...
static unsigned int dsp_hw_n3_ctrl_remap_readl(struct dsp_ctrl *ctrl,
		unsigned int addr)
{
	int ret;
	void __iomem *regs;
...
static int dsp_hw_n3_ctrl_remap_writel(struct dsp_ctrl *ctrl, unsigned int addr,
		int val)
{
	int ret;
	void __iomem *regs;
...
static unsigned int dsp_hw_n3_ctrl_sm_readl(struct dsp_ctrl *ctrl,
...
static int dsp_hw_n3_ctrl_sm_writel(struct dsp_ctrl *ctrl,
...
static unsigned int dsp_hw_n3_ctrl_dhcp_readl(struct dsp_ctrl *ctrl,
		unsigned int reg_addr)
{
	dsp_check();
...
static int dsp_hw_n3_ctrl_dhcp_writel(struct dsp_ctrl *ctrl,
		unsigned int reg_addr, int val)
{
	dsp_check();
...
static unsigned int __dsp_hw_n3_ctrl_secure_readl(struct dsp_ctrl *ctrl,
		unsigned int addr)
{
	int ret;
	unsigned long val;
...
static int __dsp_hw_n3_ctrl_secure_writel(struct dsp_ctrl *ctrl, int val,
		unsigned int addr)
{
	int ret;
...
static unsigned int dsp_hw_n3_ctrl_offset_readl(struct dsp_ctrl *ctrl,
		unsigned int reg_id, unsigned int offset)
{
	unsigned int addr;

	dsp_check();
	if (reg_id >= DSP_N3_REG_END) {
		dsp_warn("register id is invalid(%u/%u)\n",
				reg_id, DSP_N3_REG_END);
		return -EINVAL;
	}

	if (n3_sfr_reg[reg_id].flag & REG_SEC_DENY)
		return 0xdeadc000;
	else if (n3_sfr_reg[reg_id].flag & REG_WRONLY)
		return 0xdeadc001;

	addr = n3_sfr_reg[reg_id].base + n3_sfr_reg[reg_id].offset +
		offset * n3_sfr_reg[reg_id].interval;

	if (n3_sfr_reg[reg_id].flag & REG_SEC_R)
		return __dsp_hw_n3_ctrl_secure_readl(ctrl, addr);
	else
		return readl(ctrl->sfr + addr);
}

static int dsp_hw_n3_ctrl_offset_writel(struct dsp_ctrl *ctrl,
		unsigned int reg_id, unsigned int offset, int val)
{
	...
	if (n3_sfr_reg[reg_id].flag & (REG_SEC_DENY | REG_WRONLY))
		return -EACCES;

	addr = n3_sfr_reg[reg_id].base + n3_sfr_reg[reg_id].offset +
		offset * n3_sfr_reg[reg_id].interval;

	if (n3_sfr_reg[reg_id].flag & REG_SEC_W)
		__dsp_hw_n3_ctrl_secure_writel(ctrl, val, addr);
	else
		writel(val, ctrl->sfr + addr);
...
static int dsp_hw_n3_ctrl_common_init(struct dsp_ctrl *ctrl)
{
	dsp_enter();
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_SEC_MEMORY_FLAG, 0, 0xf5);
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_SEC_SLAVE_FLAG, 0, 0x6d1);

	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_CPU_REMAPS1, 0, 0x1);
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_CPU_REMAPD1, 0, 0x40340000);

	dsp_leave();
	return 0;
}

static int dsp_hw_n3_ctrl_extra_init(struct dsp_ctrl *ctrl)
{
	dsp_enter();
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_MCGEN, 0, 0xfffcffff);
	 ...
	 */
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_M2_AXUSER, 0, 0x1010);

	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_DBG_INTR_ENABLE, 0,
			0xffffffff);
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_DBG_DINTR_ENABLE, 0,
			0xffffffff);
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_DBG_MON_ENABLE, 0, 0x3);
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSP0_DBG_MON_ENABLE, 0, 0xf);
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSP0_DBG_INTR_ENABLE, 0, 0x3ffe);
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSP1_DBG_MON_ENABLE, 0, 0xf);
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSP1_DBG_INTR_ENABLE, 0, 0x3ffe);
	dsp_leave();
	return 0;
}

static int dsp_hw_n3_ctrl_all_init(struct dsp_ctrl *ctrl)
{
	dsp_enter();
...
static int dsp_hw_n3_ctrl_start(struct dsp_ctrl *ctrl)
{
	unsigned int release;

	dsp_enter();
	release = ctrl->ops->offset_readl(ctrl, DSP_N3_DSPC_CPU_RELEASE, 0);
	if (release & 0x2)
		ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_CPU_RELEASE, 0,
				release | 0x5);
	else
		ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_CPU_RELEASE, 0, 0x5);

	dsp_leave();
	return 0;
}

static int dsp_hw_n3_ctrl_reset(struct dsp_ctrl *ctrl)
{
	int ret;
	...
	while (1) {
		wfi = ctrl->ops->offset_readl(ctrl,
				DSP_N3_DSPC_CPU_WFI_STATUS, 0);
		wfe = ctrl->ops->offset_readl(ctrl,
				DSP_N3_DSPC_CPU_WFE_STATUS, 0);
	...
	if (release & 0x2)
		...
		ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_CPU_RELEASE, 0, 0x0);
...
static int dsp_hw_n3_ctrl_force_reset(struct dsp_ctrl *ctrl)
{
	dsp_enter();
	ctrl->ops->offset_writel(ctrl, DSP_N3_DSPC_HOST_MAILBOX_INTR_NS, 0, 0);
	ctrl->ops->reset(ctrl);
...
static void dsp_hw_n3_ctrl_reg_print(struct dsp_ctrl *ctrl, unsigned int reg_id)
{
...
static void __dsp_hw_n3_ctrl_pc_dump(struct dsp_ctrl *ctrl, int idx)
{
	dsp_notice("[%2d][%17s] %8x [%17s] %8x\n",
			idx, n3_sfr_reg[DSP_N3_DSPC_CPU0_PC].name,
			ctrl->ops->offset_readl(ctrl, DSP_N3_DSPC_CPU0_PC, 0),
			n3_sfr_reg[DSP_N3_DSPC_CPU1_PC].name,
			ctrl->ops->offset_readl(ctrl, DSP_N3_DSPC_CPU1_PC, 0));
	dsp_notice("[%2d][%17s] %8x\n",
			idx, n3_sfr_reg[DSP_N3_DSPC_DBG_IAC_PC].name,
			ctrl->ops->offset_readl(ctrl,
				DSP_N3_DSPC_DBG_IAC_PC, 0));
	dsp_notice("[%2d][%17s] %8x [%17s] %8x\n",
			idx, n3_sfr_reg[DSP_N3_DSP0_PERF_IVP_PC0].name,
			ctrl->ops->offset_readl(ctrl,
				DSP_N3_DSP0_PERF_IVP_PC0, 0),
			n3_sfr_reg[DSP_N3_DSP0_PERF_IVP_PC1].name,
			ctrl->ops->offset_readl(ctrl,
				DSP_N3_DSP0_PERF_IVP_PC1, 0));
	dsp_notice("[%2d][%17s] %8x [%17s] %8x\n",
			idx, n3_sfr_reg[DSP_N3_DSP1_PERF_IVP_PC0].name,
			ctrl->ops->offset_readl(ctrl,
				DSP_N3_DSP1_PERF_IVP_PC0, 0),
			n3_sfr_reg[DSP_N3_DSP1_PERF_IVP_PC1].name,
			ctrl->ops->offset_readl(ctrl,
				DSP_N3_DSP1_PERF_IVP_PC1, 0));
}

static void dsp_hw_n3_ctrl_pc_dump(struct dsp_ctrl *ctrl)
{
	int idx, repeat = 8;
	...
	for (idx = 0; idx < repeat; ++idx)
		__dsp_hw_n3_ctrl_pc_dump(ctrl, idx);

	dsp_leave();
}

static void dsp_hw_n3_ctrl_dhcp_dump(struct dsp_ctrl *ctrl)
{
	int idx;
	unsigned int addr;
	...
	dsp_notice("reserved shared mem dump (count:%d)\n",
			DSP_N3_SM_USED_COUNT);
	for (idx = 0; idx < DSP_N3_SM_USED_COUNT; idx += 4) {
		addr = DSP_N3_SM_RESERVED(idx);
		val0 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_RESERVED(idx));
		val1 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_RESERVED(idx + 1));
		val2 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_RESERVED(idx + 2));
		val3 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_RESERVED(idx + 3));
...
static void dsp_hw_n3_ctrl_userdefined_dump(struct dsp_ctrl *ctrl)
{
	int idx;
	unsigned int addr;
	unsigned int val0, val1, val2, val3;
	...
	for (idx = 0; idx < DSP_N3_SM_USERDEFINED_COUNT; idx += 4) {
		addr = DSP_N3_SM_USERDEFINED(idx);
		val0 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_USERDEFINED(idx));
		val1 = ctrl->ops->sm_readl(ctrl,
				DSP_N3_SM_USERDEFINED(idx + 1));
		val2 = ctrl->ops->sm_readl(ctrl,
				DSP_N3_SM_USERDEFINED(idx + 2));
		val3 = ctrl->ops->sm_readl(ctrl,
				DSP_N3_SM_USERDEFINED(idx + 3));
...
static void dsp_hw_n3_ctrl_fw_info_dump(struct dsp_ctrl *ctrl)
{
	int idx;
	unsigned int addr;
	unsigned int val0, val1, val2, val3;
	...
	dsp_enter();
	dsp_notice("fw_info dump (count:%d)\n", DSP_N3_SM_FW_INFO_COUNT);
	for (idx = 0; idx < DSP_N3_SM_FW_INFO_COUNT; idx += 4) {
		addr = DSP_N3_SM_FW_INFO(idx);
		val0 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_FW_INFO(idx));
		val1 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_FW_INFO(idx + 1));
		val2 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_FW_INFO(idx + 2));
		val3 = ctrl->ops->sm_readl(ctrl, DSP_N3_SM_FW_INFO(idx + 3));
...
static void dsp_hw_n3_ctrl_dump(struct dsp_ctrl *ctrl)
{
	int idx;

	dsp_enter();
	dsp_notice("register dump (count:%d)\n", DSP_N3_REG_END);
	for (idx = 0; idx < DSP_N3_REG_END; ++idx)
...
static void dsp_hw_n3_ctrl_user_reg_print(struct dsp_ctrl *ctrl,
...
static void dsp_hw_n3_ctrl_user_pc_dump(struct dsp_ctrl *ctrl,
		struct seq_file *file)
{
	...
	for (idx = 0; idx < repeat; ++idx) {
		seq_printf(file, "[%2d][%17s] %8x [%17s] %8x\n",
				idx, n3_sfr_reg[DSP_N3_DSPC_CPU0_PC].name,
				ctrl->ops->offset_readl(ctrl,
					DSP_N3_DSPC_CPU0_PC, 0),
				n3_sfr_reg[DSP_N3_DSPC_CPU1_PC].name,
				ctrl->ops->offset_readl(ctrl,
					DSP_N3_DSPC_CPU1_PC, 0));
...
static void dsp_hw_n3_ctrl_user_dhcp_dump(struct dsp_ctrl *ctrl,
		struct seq_file *file)
{
	int idx;
	unsigned int addr;
...
static void dsp_hw_n3_ctrl_user_userdefined_dump(struct dsp_ctrl *ctrl,
		struct seq_file *file)
{
	int idx;
	unsigned int addr;
...
static void dsp_hw_n3_ctrl_user_fw_info_dump(struct dsp_ctrl *ctrl,
		struct seq_file *file)
{
	int idx;
	unsigned int addr;
...
static void dsp_hw_n3_ctrl_user_dump(struct dsp_ctrl *ctrl,
		struct seq_file *file)
{
	int idx;
...
static int dsp_hw_n3_ctrl_open(struct dsp_ctrl *ctrl)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_n3_ctrl_close(struct dsp_ctrl *ctrl)
{
	dsp_enter();
	dsp_leave();
	return 0;
...
static int dsp_hw_n3_ctrl_probe(struct dsp_system *sys)
{
...
static void dsp_hw_n3_ctrl_remove(struct dsp_ctrl *ctrl)
{
	dsp_enter();
	dsp_leave();
}

static const struct dsp_ctrl_ops n3_ctrl_ops = {
	.remap_readl		= dsp_hw_n3_ctrl_remap_readl,
	.remap_writel		= dsp_hw_n3_ctrl_remap_writel,
	.sm_readl		= dsp_hw_n3_ctrl_sm_readl,
	.sm_writel		= dsp_hw_n3_ctrl_sm_writel,
	.dhcp_readl		= dsp_hw_n3_ctrl_dhcp_readl,
	.dhcp_writel		= dsp_hw_n3_ctrl_dhcp_writel,
	.offset_readl		= dsp_hw_n3_ctrl_offset_readl,
	.offset_writel		= dsp_hw_n3_ctrl_offset_writel,

	.reg_print		= dsp_hw_n3_ctrl_reg_print,
	.dump			= dsp_hw_n3_ctrl_dump,
	.pc_dump		= dsp_hw_n3_ctrl_pc_dump,
	.dhcp_dump		= dsp_hw_n3_ctrl_dhcp_dump,
	.userdefined_dump	= dsp_hw_n3_ctrl_userdefined_dump,
	.fw_info_dump		= dsp_hw_n3_ctrl_fw_info_dump,

	.user_reg_print		= dsp_hw_n3_ctrl_user_reg_print,
	.user_dump		= dsp_hw_n3_ctrl_user_dump,
	.user_pc_dump		= dsp_hw_n3_ctrl_user_pc_dump,
	.user_dhcp_dump		= dsp_hw_n3_ctrl_user_dhcp_dump,
	.user_userdefined_dump	= dsp_hw_n3_ctrl_user_userdefined_dump,
	.user_fw_info_dump	= dsp_hw_n3_ctrl_user_fw_info_dump,

	.common_init		= dsp_hw_n3_ctrl_common_init,
	.extra_init		= dsp_hw_n3_ctrl_extra_init,
	.all_init		= dsp_hw_n3_ctrl_all_init,
	.start			= dsp_hw_n3_ctrl_start,
	.reset			= dsp_hw_n3_ctrl_reset,
	.force_reset		= dsp_hw_n3_ctrl_force_reset,

	.open			= dsp_hw_n3_ctrl_open,
	.close			= dsp_hw_n3_ctrl_close,
	.probe			= dsp_hw_n3_ctrl_probe,
	.remove			= dsp_hw_n3_ctrl_remove,
};
...
int dsp_hw_n3_ctrl_init(void)
{
	...
	dsp_enter();
	ret = dsp_ctrl_register_ops(DSP_DEVICE_ID_N3, &n3_ctrl_ops);
	if (ret)
		goto p_err;
