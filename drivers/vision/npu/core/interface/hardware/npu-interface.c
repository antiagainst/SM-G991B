...
static void __send_interrupt(u32 cmdType, struct command *cmd)
{
	u32 val;
	u32 timeout;
...
static int npu_set_cmd(struct message *msg, struct command *cmd, u32 cmdType)
{
	...
	if (ret)
		goto I_ERR;
	__send_interrupt(msg->command, cmd);
	return 0;
I_ERR:
	switch (ret) {
	case -ERESOURCE:
...
int npu_interface_open(struct npu_system *system)
{
	...
	interface.addr = (void *)((npu_get_mem_area(system, "fwmemory")->vaddr) + configs[NPU_MAILBOX_BASE]);
