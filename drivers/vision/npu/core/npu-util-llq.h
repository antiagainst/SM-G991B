...
struct llq_time_stat {
	struct timespec64	ts_init;	/* First initialization */
	struct timespec64	ts_attempt_get;	/* Last invocation of GET */
	struct timespec64	ts_success_get;	/* Last successful invcation of GET */
	struct timespec64	ts_attempt_put;	/* Last invocation of PUT */
	struct timespec64	ts_success_put;	/* Last successful invcation of PUT */
	struct timespec64	ts_is_empty;	/* Last invocation of is_available */
};

/* Time stamp update */
#define __LLQ_UPDATE_STAT(QNAME, TARGET_STAT)					\
do {										\
	((QNAME).stat.TARGET_STAT) = ktime_to_timespec64(ktime_get_boottime());	\
} while (0)									\

/* Convinient macro for setting two timestamp at once */
#define __LLQ_UPDATE_STAT2(QNAME, TARGET_STAT_1, TARGET_STAT_2)			\
do {										\
	((QNAME).stat.TARGET_STAT_1) = ktime_to_timespec64(ktime_get_boottime());	\
