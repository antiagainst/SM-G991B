...
static int  npu_protodrv_handler_nw_requested(void)
{
	...
	LSM_FOR_EACH_ENTRY_IN(proto_nw_lsm, REQUESTED, entry,
		...
		case NPU_NW_CMD_POWER_DOWN:
			...
			if (likely(!is_session_ref_exist())) {
				if (likely(__mbox_nw_ops_put(entry) > 0)) {
					/* Success */
					proto_nw_lsm.lsm_move_entry(PROCESSING, entry);
					proc_handle_cnt++;
...
{
	int ret = 0;
	unsigned int task_cpu_affinity = 0;
	...
	if (unlikely(!npu_proto_drv.if_mbox_ctx)) {
	...
	ret = of_property_read_u32(npu_device->dev->of_node,
		"samsung,npuproto-task-cpu-affinity", &task_cpu_affinity);
	if (unlikely(ret))
		task_cpu_affinity = 0;
	npu_info("set the CPU affinity of AST to %u\n", task_cpu_affinity);
	ret = set_cpus_allowed_ptr(npu_proto_drv.ast.thread_ref, cpumask_of(task_cpu_affinity));
	if (unlikely(ret))
