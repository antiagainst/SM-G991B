...
static void dump_auto_sleep_thread(const struct auto_sleep_thread *ctx)
{
	...
	npu_dbg("state = %ld\n", ctx, ctx->thread_ref->state);
	npu_dbg("pid = %u\n", ctx, ctx->thread_ref->pid);
	npu_dbg("tgid = %u\n", ctx, ctx->thread_ref->tgid);
	npu_dbg("set_child_tid = %pK\n", ctx, ctx->thread_ref->set_child_tid);
	npu_dbg("name = %s\n", ctx, ctx->name);
	npu_dbg("do_task = %pK\n", ctx, ctx->do_task);
	npu_dbg("task_param = {.data = %pK}\n", ctx, ctx->task_param.data);
	npu_dbg("no_activity_threshold = %d\n", ctx, ctx->no_activity_threshold);
	npu_dbg("thr_state = %d\n", ctx, atomic_read(&(ctx->thr_state)));
}
