...
static int __npu_golden_compare(const struct npu_frame *frame)
{
	...
	list_for_each_entry(golden_entry, &npu_golden_ctx.data.file_list, list) {
		...
		/* Cache invalidation */
		//dma_sync_single_for_cpu(npu_golden_ctx.dev,
		//	target_entry->daddr, target_entry->size, DMA_FROM_DEVICE);
...
int register_golden_matcher(struct device *dev)
{
	...
	probe_info("start in register_golden_matcher\n");

	BUG_ON(!dev);

	probe_info("initialize statekeeper in register_golden_matcher\n");
	npu_statekeeper_initialize(&npu_golden_ctx.statekeeper,
				   NPU_GOLDEN_STATE_INVALID,
				   state_names, transition_map);

	probe_info("Registeration of golden matcher : Register " SET_GOLDEN_DESC_DEBUGFS_NAME " to debugfs.\n");
	ret = npu_debug_register(SET_GOLDEN_DESC_DEBUGFS_NAME, &npu_golden_set_debug_fops);
	if (ret) {
		probe_err("fail(%d) in npu_debug_register for " SET_GOLDEN_DESC_DEBUGFS_NAME "\n", ret);
		goto err_exit;
	}

	probe_info("Registeration of golden matcher: Register " RESULT_GOLDEN_MATCH_DEBUGFS_NAME " to debugfs.\n");
	ret = npu_debug_register(RESULT_GOLDEN_MATCH_DEBUGFS_NAME, &npu_golden_match_debug_fops);
	if (ret) {
		probe_err("fail(%d) in npu_debug_register for " RESULT_GOLDEN_MATCH_DEBUGFS_NAME "\n", ret);
		goto err_exit;
	}
	...
	probe_info("complete in register_golden_matcher\n");
	return 0;
