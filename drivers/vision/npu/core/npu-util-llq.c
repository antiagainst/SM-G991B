...
static inline s64 timespec_sub_to_ms(const struct timespec64 *lhs, const struct timespec64 *rhs)
{
	struct timespec64 ts;

	ts = timespec64_sub(*lhs, *rhs);
	return timespec64_to_ns(&ts) / NS_TO_MS;
}
...
size_t llq_print_timestat(const struct llq_time_stat *stat, char *out_buf, const size_t len)
{
	...
	ts_now = ktime_to_timespec64(ktime_get_boottime());
