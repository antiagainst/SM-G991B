...
int npu_store_log(npu_log_level_e loglevel, const char *fmt, ...)
{
	...
	if ((npu_log.line_cnt & NPU_STORE_LOG_SYNC_MARK_INTERVAL_MASK) == 0)
		npu_info(NPU_STORE_LOG_SYNC_MARK_MSG, npu_log.line_cnt);
...
void npu_log_on_error(void)
{
	...
	if (ret)
		npu_err("%s(): npu_store_log_dump() failed: ret = %d\n", __func__, ret);
}
...
{
	size_t n = 0;
	struct npu_log_unit *ptr;

	if (src_size < sizeof(struct npu_log_unit))
		return 0;
...
struct npu_log_unit *npu_set_string(struct npu_log_unit *npu_log_ptr, const char *fmt, ...)
{
	va_list ap;
...
int npu_log_release(struct npu_device *npu_device)
{
	...
	/* todo release allocated memory area for logging */
	// memlog_free();
