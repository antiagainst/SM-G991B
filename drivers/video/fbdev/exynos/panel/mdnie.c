...
int mdnie_probe(struct panel_device *panel, struct mdnie_tune *mdnie_tune)
{
	...
	if (panel->id == 0)
		snprintf(name, MAX_MDNIE_DEV_NAME_SIZE,
				"%s", MDNIE_DEV_NAME);
	else
		snprintf(name, MAX_MDNIE_DEV_NAME_SIZE,
				"%s-%d", MDNIE_DEV_NAME, panel->id);

	mdnie->class = class_create(THIS_MODULE, name);
...
int mdnie_remove(struct panel_device *panel)
{
	struct mdnie_info *mdnie = &panel->mdnie;
	int i;
...
static int attr_find_and_show(struct device *dev,
...
ssize_t attr_show_for_each(struct class *cls,
	const char *name, char *buf)
{
	struct class_dev_iter iter;
	struct device *dev;
	...
	class_dev_iter_init(&iter, class, NULL, NULL);
	while ((dev = class_dev_iter_next(&iter))) {
		error = attr_find_and_show(dev, name, buf);
		if (error)
			break;
	}
	class_dev_iter_exit(&iter);
