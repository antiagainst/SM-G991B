/*
 * linux/drivers/video/fbdev/exynos/panel/s6e3fab/s6e3fab_unbound1_aod_panel.h
 *
...
#ifndef __S6E3FAB_UNBOUND1_AOD_PANEL_H__
#define __S6E3FAB_UNBOUND1_AOD_PANEL_H__

#include "s6e3fab_aod.h"
#include "s6e3fab_aod_panel.h"

#include "s6e3fab_unbound1_self_mask_img.h"
#include "s6e3fab_self_analog_clock_img.h"
#include "s6e3fab_self_digital_clock_img.h"

/* UNBOUND1 */
static DEFINE_STATIC_PACKET_WITH_OPTION(s6e3fab_unbound1_aod_self_mask_img_pkt,
		DSI_PKT_TYPE_WR_SR, UNBOUND1_SELF_MASK_IMG, 0, PKT_OPTION_SR_ALIGN_16);

static void *s6e3fab_unbound1_aod_self_mask_img_cmdtbl[] = {
	&KEYINFO(s6e3fab_aod_l1_key_enable),
	&PKTINFO(s6e3fab_aod_self_mask_sd_path),
	&DLYINFO(s6e3fab_aod_self_spsram_sel_delay),
	&PKTINFO(s6e3fab_unbound1_aod_self_mask_img_pkt),
	&DLYINFO(s6e3fab_aod_self_spsram_1ms_delay),
	&PKTINFO(s6e3fab_aod_reset_sd_path),
	&KEYINFO(s6e3fab_aod_l1_key_disable),
};

static struct seqinfo s6e3fab_unbound1_aod_seqtbl[MAX_AOD_SEQ] = {
	[SELF_MASK_IMG_SEQ] = SEQINFO_INIT("self_mask_img", s6e3fab_unbound1_aod_self_mask_img_cmdtbl),
...
static struct aod_tune s6e3fab_unbound1_aod = {
	.name = "s6e3fab_unbound1_aod",
	.nr_seqtbl = ARRAY_SIZE(s6e3fab_unbound1_aod_seqtbl),
	.seqtbl = s6e3fab_unbound1_aod_seqtbl,
	.nr_maptbl = ARRAY_SIZE(s6e3fab_aod_maptbl),
	.maptbl = s6e3fab_aod_maptbl,
	.self_mask_en = true,
};
#endif //__S6E3FAB_UNBOUND1_AOD_PANEL_H__
