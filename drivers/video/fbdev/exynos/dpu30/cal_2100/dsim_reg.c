...
static void dsim_reg_sw_reset(u32 id)
{
	...
	ret = readl_poll_timeout(dsim->res.regs + DSIM_SWRST, val,
...
static int dsim_reg_wait_phy_ready_clane(u32 id, bool en)
{
	...
	ret = readl_poll_timeout(dsim->res.phy_regs + DSIM_PHY_MC_GNR_CON0,
...
static int dsim_reg_wait_phy_ready_dlane(u32 id, u32 lane_id, bool en)
{
	...
	ret = readl_poll_timeout(dsim->res.phy_regs + reg_id, val,
...
static int dsim_reg_enable_pll(u32 id, u32 en)
{
	...
	ret = readl_poll_timeout(dsim->res.phy_regs + DSIM_PHY_PLL_STAT0,
			val,
			en ? DSIM_PHY_PLL_LOCK_GET(val) :
			!DSIM_PHY_PLL_LOCK_GET(val),
			10, 2000);
	if (ret) {
...
void dpu_sysreg_select_dphy_rst_control(void __iomem *sysreg, u32 dsim_id,
		u32 sel)
{
	u32 phy_num = dsim_id;
...
void dsim_reg_preinit(u32 id)
{
	...
	lcd_info.xres = 1080;
	lcd_info.yres = 1920;
	lcd_info.dphy_pms.p = 0x02;
	lcd_info.dphy_pms.m = 0x8A;
	lcd_info.dphy_pms.s = 0x01;
	lcd_info.data_lane = 4;
	lcd_info.cur_mode_idx = 0;
	idx = lcd_info.cur_mode_idx;
	lcd_info.display_mode[idx].cmd_lp_ref = 3022;
#if IS_ENABLED(CONFIG_MCD_PANEL)
	lcd_info.cmd_lp_ref = 3022;
#endif
...
void dsim_reg_function_reset(u32 id)
{
	...
	ret = readl_poll_timeout(dsim->res.regs + DSIM_SWRST, val,
			!(val & DSIM_SWRST_FUNCRST), 10, 2000);
	if (ret)
		dsim_err("dsim%d function reset timeout\n", id);
}
...
void dsim_reg_set_cmd_access_mode(u32 id, u32 dma_en)
{
	...
	dsim_write_mask(id, DSIM_CMD_MODE, val, DSIM_CMD_ACCESS_MODE_MASK);
}
...
u32  dsim_reg_get_cmd_access_mode(u32 id)
{
	...
	val = dsim_read(id, DSIM_CMD_MODE);
