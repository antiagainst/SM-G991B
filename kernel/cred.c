...
#ifdef CONFIG_KDP_CRED
	ret = is_kdp_protect_addr((unsigned long)cred);

	if (ret == PROTECT_INIT)
		BUG_ON(atomic_read(init_cred_kdp.use_cnt) != 0);
	else if (ret == PROTECT_KMEM)
		BUG_ON(atomic_read(((struct cred_kdp *)cred)->use_cnt) != 0);
	else
#endif
...
#ifdef CONFIG_KDP_CRED
	if (kdp_enable) {
		volatile unsigned int kdp_use_count = kdp_get_usecount(new);
		struct cred *new_ro;

		new_ro = prepare_ro_creds(new, CMD_OVRD_CREDS, kdp_use_count);
		*cnew = new_ro;
